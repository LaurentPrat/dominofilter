...
STATUS		GetFieldInList(NOTEHANDLE note_handle, char* FieldName, char** field_List, int MAXItemSize, int* nbitem);
                    ...
                    if (IDIsPresent(idtable, note_id))
					{
						...
						*/
						...
						if (MW_SKIPRULE[0] == '\0' || MW_SKIPRULE[0] == '2')
						{
							...
							NSFItemDelete(hNote, MW_MACRO_FIELD_NB_RECIPIENTS,(WORD) strlen(MW_MACRO_FIELD_NB_RECIPIENTS)); /* 3.94.46 */
	...
	*/
	...
	if (advarch || advblck || advform  || advquota)
	{
		...
		if (advblck)
		{
			...
			if (error = NIFOpenCollection(hDbRules, hDbRules, viewmb_id, 0,
...
STATUS DeleteAttachFileByName(NOTEHANDLE note_handle, char* AttachedFileName)
{
 ...
 if ( error = GetFieldInList(note_handle, AttachedFileName, FileNameList, MAXPATH, &nbitem) )
 {
	return( error );
 }
										...
										int iUnderline, char sep)
{
	...
	if ( error = GetFieldInList(hNote, sFieldNameRef, sFieldValue1, MAXPATH, &nbitem) )
		return(error);
										...
										int iUnderline, char sep)
{
	...
	/* --------------------------------------------------------------*/
	/* Get TEXT, TEXT LIST - FIELD 1 */
	if ( error = GetFieldInList(hNote, sTextItemFieldName1, sFieldValue1, MAXPATH, &nbitem1) )
		return(error);

	/* Get TEXT, TEXT LIST - FIELD 2 */
	if ( error = GetFieldInList(hNote, sTextItemFieldName2, sFieldValue2, MAXPATH, &nbitem2) )
	{
	...
	if ( error = GetFieldInList(hNote, sTextItemFieldName3, sFieldValue3, MAXPATH, &nbitem3) )
	{
		FreeMemory_GetFieldInList(sFieldValue1, nbitem1);
										...
										char* HeaderTitle, int iUnderline, char sep)
{
	...
	/* Get TEXT, TEXT LIST */
	if ( error = GetFieldInList(hNote, sTextItemFieldName, sFieldValue, MAXPATH, &nbitem) )
		return(error);
											...
											char* sFieldRef)
{
	...
	/* Get List */
	if ( error = GetFieldInList(hNoteFiltre, sFieldRef, sFieldValue, MAXPATH, &nbitem) )
		return(error);
	...
	if ( nbitem > 0 )
	{
		...
		/* Get Title TEXT List */
		error = GetFieldInList(hNoteFiltre, sHeaderTitle, sTitleValue, MAXPATH, &nbitem_title);
