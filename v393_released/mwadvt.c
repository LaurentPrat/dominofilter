 ...
 *
 * 16/12/04				: Revision 3.93.28
 *						: remove AlerteAdmin(1)
 *						:
 *						: Revision 3.93.29
 *						: Add debug fonction in ProcBlackList
 *						:
 * 17/12/04				: Revision 3.93.30
 *						: Fix : OverQuota - use Resend Button & UpdateBody.
 *
 * 29/12/04				: Revision 3.93.31
 *						: Fix : MakeStringFromNotesAddressList()
 *						: change in : AppendMailHeaderToBody() : display "CC" as well as "SendTo"
 *
 * 07/01/05				: Revision 3.93.32
 *						: Add parameter for OverQuota process : AdvQuota_ResendAction_FormName
 *						: change in : OverQuota, OpenDbs
 *						: new global var : AdvQuota_ResendAction_FormName ( char [100] )
 *						: add/remove icone into/from the return message : INBOX_ICON
 *					
 * 28/02/05				: Revision 3.93.33
 ...
 *						:
 * 16/03/05				: Revision 3.93.34/35
 *						: new argument is OverQuota ( NOTEHANDLE* hCopy, int* IsNewDoc )
 *						: append mw_ before ( NSFNoteGetSize( )) - conflict during link - AIX
 * 23/03/05				: Revision 3.93.36
 *						: Remove '$KeepPrivate="1"' when OverQuota is sent back restore it on the resend
 * 18/04/05				: Revision 3.93.37
 *						: OverQuota : IntendedRecipients : SendTo/CopyTo/BCC should be the same.
 * 16/05/05				: revision 3.93.38				
 *						: fix in AddMessageToEmail: prevent CompoundTextClose if no discard.
 *						: fix in ProcBlackList: bRecOk = 0 instead of bRecOk = 1;
 * 28/06/05				: revision 3.93.39
 *						: change in SimpleAppendDocLink : (3.93.39,40) URL formatting
 * 13/06/05				: revsision 3.93.41 : reset URL - remove (3.93.39,40) - stand by 
 * 14/09/05				: revision 3.93.42
 *						: remove the intialisation by default for ( BList_DF_Header, "Re : ");
 *						: change in ProcBlackList
 * --[ TO DO ]-------------------------------------------------------------------------------------------------------
 **************************************************************************************************/
...
#include "mwadvt.h"
#include "wildcard.h"
#include "mwlist.h"		/* rename for AS400 */
...
BOOL		bMatch(char, char);
int			GetPos(char* a, char x);
int			ProcBlackList(NOTEHANDLE *, DBHANDLE,char* mail_ID);
STATUS		GetBlackRedListRules(HCOLLECTION coll_bl);
void		AlerteAdmin(int);
void		SplitGroup(NOTEHANDLE, char*);
int			ProcFormula(NOTEHANDLE);
BOOL		EvalFormula(NOTEHANDLE, char *, char *);
void		ProcCrochet(NOTEHANDLE);
void		dumpstat(void);
...
STATUS		SimpleAppendDocLink(TIMEDATE, UNID, UNID, char*, HANDLE);
void		SimpleProcCrochet_DocLink(NOTEHANDLE, char*, STATUS, STATUS,int*, char*, TIMEDATE*, UNID*, UNID*, char*,BOOL);
void		NonDeliveryBlackList_DocLink(DBHANDLE, NOTEHANDLE, char*,char*,char*,char*, char*,char*, NOTEHANDLE, int, char*, TIMEDATE, UNID, UNID, char*, DBHANDLE*, char*, char*,char*);
...
int			GetUserAddressFullName(char* user_address, char* user_fullname, char* fieldname);
...
BOOL		advstat, advstat_subject, advarch, advblck, advform, advnotify_blacklist;
...
/* 3.93.29 DEBUG MODE */
#define MAXDEBUGLINE							(500)
#define	DEFAULT_PROCBLACKLIST_DUMP_FILE			"mwblacklistdebug.txt"
...
STATUS far PASCAL  AddInMain (HMODULE hModule, int argc, char *argv[])
{
	...
	/* Initialisation. On ouvre les bases ainsi que les vues a utiliser */
	/* On commence par regarder si la passerelle n'est pas déjà lancée */
	...
	/** 3.93.29 Debug per procedure **/
	MWBLACKLISTDEBUG = FALSE;
	if (OSGetEnvironmentString("MWDEBUGBLACKLIST", szDomain, sizeof(szDomain)-1))
	...
	if (sErreur = OpenDbs())
	{
		/* NO the variable may not be intialized Alerte    : Admin(1); */
		return(NOERROR);
	}
	...
	if (advblck)
	{
		...
		if (advblck) /* advblck is reset to FALSE if an error has occured during OpenNAB */
		{
				...
				if (sErreur = GetBlackRedListRules(coll_bl))
					AlerteAdmin(4);
	...
	while (!AddInIdle())
	{
		...
		if (MW_RefreshView > -1)
		{
			 ...
			 {
				AddInLogMessageText(UPDATE_VIEW, NOERROR, NOMAPI, dbsetup);
						...
						*/
						...
						if (QuotaEnd == 0)
						{
							...
							if (advstat)
							{
								...
								if ( ! NSFItemGetText(hNote, MW_MAILDUPLICATE, qui, sizeof(qui)-1 ))
								{
									mailduplicate = 0;
									...
									sprintf(statmw[lgstat].Time, "%02d:%02d", current.hour, current.minute);
									...
									sprintf(statmw[lgstat].PostedTime, "%02d:%02d", current.hour, current.minute);
									...
									/* Form field */
									NSFItemGetText(hNote, "Form", qui, sizeof(qui)-1);
									strcpy(statmw[lgstat].Form, qui);

									/* taille + nb objets */
									num = mw_NSFNoteGetSize(hMb, hNote, &pj);
									...
									if (!NSFItemGetText(hNote, MW_DOCID_TRACKER, MW_DocID, sizeof(MW_DocID)-1))
									{
										...
										/* for the stat this document has not ancestor */
										strcpy(MW_DocID, "0");
									}
									/* write data to stat struct */
									strcpy(statmw[lgstat].MW_DocID, MW_DocID);
									/* ---------------------------------------------------------------------*/
							...
							{
								/** if Returns 1 => Delete **/
								if (ProcBlackList(&hNote, hMb, statmw[lgstat].DocID) == 1)
								{
...
STATUS OpenDbs(void)
{
	...
	if (error = NIFOpenCollection(hConfdb, hConfdb, vca_id, OPEN_REBUILD_INDEX,
	...
	if (error = NIFFindByName(coll_ca, "Mail_Watcher",
	...
	if (error = NIFReadEntries(coll_ca, &coll_capos, NAVIGATE_CURRENT, 0,
	...
	if (advarch || advblck || advform  || advquota)
	{
	  ...
	  if (!EstVraiBase(hDb))
	  {
		...
		if (error = NIFFindDesignNote(hDb, "($BlackList)", NOTE_CLASS_VIEW, &viewmb_id))
		  ...
		  if (error = NIFOpenCollection(hDb, hDb, viewmb_id, 0,
										NULLHANDLE, &coll_bl, NULL, NULL,
										NULL, NULL))
		  {
				NSFNoteClose(hConfNote);
				OSUnlockObject(buffer_cahandle);
		  ...
		  /* Get the HCOLLECTION Handle */
		  if (error = NIFOpenCollection(hDb, hDb, viewmb_id, 0,
										NULLHANDLE, &coll_bl_formula, NULL, NULL,
										NULL, NULL))
		  {
				NSFNoteClose(hConfNote);
				OSUnlockObject(buffer_cahandle);
		...
		if (advarch)
		{
		  ...
		  if (error = NIFFindDesignNote(hDb, "($Arch)", NOTE_CLASS_VIEW, &viewmb_id))
		  {
				...
				NIFCloseCollection(coll_bl); /* rules with priority order */
				NSFDbClose(hDb);
				AddInLogMessageText(ERR_FIND_VIEW, error, NOMAPI,
											"($Arch)", dbsetup);
				return(error);
		  }

			if (error = NIFOpenCollection(hDb, hDb, viewmb_id, 0,
												NULLHANDLE, &coll_cr, NULL, NULL,
												NULL, NULL))
		  {
				NSFNoteClose(hConfNote);
				OSUnlockObject(buffer_cahandle);
				...
				NIFCloseCollection(coll_bl); /* rules with priority order */
				NSFDbClose(hDb);
				AddInLogMessageText(ERR_OPEN_VIEW, error, NOMAPI,
												"($Arch)", dbsetup);
				return(error);
		  }
		}
...
int OverQuota(NOTEHANDLE* hNote, NOTEHANDLE* hCopy, int* IsNewDoc, DBHANDLE hDB, BOOL advquota_resend)
{
	...
	if (QuotaFilter(*hNote, emetteur))
	{
		...
		/* 3.93.37 : Move SendTo in IntendedRecipients, do not copy CopyTo;BCC as they are part of Recipients */
		if ( UpdateBody == 0 )			
		...
		NSFItemSetText(*hNote, "Recipients", emetteur, (WORD) strlen(emetteur));
	...
	else
	{
		...
		if ( NSFItemIsPresent(*hNote, OVERQUOTA_FORM, (WORD) strlen(OVERQUOTA_FORM) ))
		{
			/* 3.93.32 + 3.93.36 remove Icon & other temporary fields */
 ...
 *
 *****************************************************************************************************/
int ProcBlackList(NOTEHANDLE *hNote, DBHANDLE hDB, char* mail_ID)
{
	...
	if ( MWBLACKLISTDEBUG )
		WriteToDebugLog(DEFAULT_PROCBLACKLIST_DUMP_FILE, *hNote, "MW_BLMemo");
	...
	if ( MWBLACKLISTDEBUG )
	{
		sprintf(LogString, "NbRec : %i ", nbrec );
		LogLine(DEFAULT_PROCBLACKLIST_DUMP_FILE, LogString, 1);
	}
	...
	for (i=0; i<nbrec; i++)
	{
		...
		bRecOK   = 0;		/* 3.93.38 : recipient is not blacklisted by default : is blacklisted if ends up with bRecOk < 0 */
		...
		while (BL_Rules < MaxRules)
		{
			...
			{
				/* debug - dump : match rules */
				if ( MWBLACKLISTDEBUG )
				{
					sprintf(LogString, "FROM : %s ", emetteurabr );
					LogLine(DEFAULT_PROCBLACKLIST_DUMP_FILE, LogString, 1);

					sprintf(LogString, "TO   : %s ",  recipientabr );
					LogLine(DEFAULT_PROCBLACKLIST_DUMP_FILE, LogString, 1);

					sprintf(LogString, "MATCH : %s ", clef );
					LogLine(DEFAULT_PROCBLACKLIST_DUMP_FILE, LogString, 1);
				}
				...
				if (Except_Match(emetteurabr, recipientabr, BlackRedList[BL_Rules -1]) == 0)
				{
					...
					if (sErreur)
					{
						...
						AddInLogMessageText(ERR_READ_VIEW, sErreur, NOMAPI, "FindByName : key not found", dbsetup);
					...
					{
		 				NSFItemAppendTextList(*hNote, "RecOK", recipient, (WORD) strlen(recipient), FALSE);
						AddInLogMessageText(ERR_READ_VIEW, sErreur, NOMAPI, "($BlackList_Formula)", dbsetup);
        ...
        /*=========================[ TEST IF THE RULES # has been applied yet or not ]==========*/
						...
						sprintf(RefRules, "%d,",BL_Rules);	/* should be BL_Rules -1 : 3.93.38 */
						ApplyAction = 0;
						...
						/* === [          Physical change : need to duplicate the mail  ]              ==== */
							...
							/* The rule definition contains a flags : if set to 1, we need to work out on a new "memory" copy    */
							/* of the current hnote (the original email)                                                         */
								...
								if (MailCopyBool)
								{
									...
									if (NSFItemGetText(hNoteFiltre, "BList_IncludeDocLink", bl_choice, sizeof(bl_choice)-1))
									{
										...
										if (DocLink == 1)
											...
											SimpleProcCrochet_DocLink(hNoteSafe, MailCopy, MW_RAPP_Bool, MW_RAPP_AGAIN_Bool, &DocLink, ViewName, &replica_ID, &Note_UNID, &View_UNID, FolderName, RemoveReturnReceipt);
										else
						...
						if (NSFItemGetText(hNoteFiltre, "BList_DFMail", bl_choice, sizeof(bl_choice)-1))
						{
							if (strcmp(bl_choice,"1") == 0)
							{
								...
								if (!NSFItemGetText(hNoteFiltre, "BList_DF_Header", BList_DF_Header, sizeof(BList_DF_Header)-1))
									BList_DF_Header[0] = '\0'; /* 3.93.42 : set to NULL */
								...
								/* Call the BlackListDeliveryFailure */
								if (DocLink == 1)
									NonDeliveryBlackList_DocLink(hDB, *hNote, NewEmailFromAddress, blacklister, sendToBis, NewMemo_Fwd, BList_DF_Header, OrigSubject, hNoteFiltre, DocLink, ViewName, replica_ID,
										Note_UNID, View_UNID, Original_Body, &hDB, emetteur, safe_recipient,Original_Body_Attach);
								else
				...
				else
				{
					...
					if ( MWBLACKLISTDEBUG )
					{
						sprintf(LogString, "%s", "EXCEPTION MATCH" );
						LogLine(DEFAULT_PROCBLACKLIST_DUMP_FILE, LogString, 1);
					}
		...
		if (bRecOK < 0)
			printf("(=== END ===) %s is Blacklisted, up to :%d \n", safe_recipient, bRecOK);
		else
			printf("(=== END ===) %s is -- OK -- up to : %d \n", safe_recipient, bRecOK);
#endif
		...
		else
		{
			...
			in the original document not in a new copy */
			if (bRecOK < 0)
			{
				bRecNotOk = 1;
...
BOOL EvalFormula(NOTEHANDLE hNote, char *formula, char* noteid)
{
	...
	if (sErreur)
	{
		AddInLogMessageText(ERR_COMPILE_FORMULA, sErreur, NOMAPI, noteid, dbsetup, rcErreurO);
		return(FALSE);
	}
	...
	if (sErreur)
	{
		...
		OSMemFree(hFormula);
		AddInLogMessageText(ERR_COMPUTE_START, sErreur, NOMAPI, noteid, dbsetup);
		return(FALSE);
	}
 ...
 * Output : void
 *********************************************************************/
void ProcCrochet(NOTEHANDLE hNote)
{
	STATUS sErreur;
	char emetteur[MAXUSERNAME];
	char emetteurabr[MAXUSERNAME];
	...
	for (ctr=0; ctr < nbelem; ctr++)
	{
									...
									&buffer_fhandle, NULL, NULL, &atraiter, NULL))
		{
			...
			AddInLogMessageText(ERR_READ_VIEW, sErreur, NOMAPI, "($Crochet)", dbsetup);
			continue;
		}
		...
		if ((buffer_fhandle == NULLHANDLE) || (atraiter == 0))
		{
			...
			AddInLogMessageText(ERR_OPEN_FILTER_DOC, sErreur, NOMAPI, 0x0, dbsetup);
			continue;
		}
...
/****** Sub - Returns True if a matches b *****/
BOOL bMatch(char a, char b)
{
			  	...
			  	for (i= 0; i< (int) NotesFound; i++)
				{
					...
					else
					{
						...
						if (!NSFItemGetText(note_handle, "formule", filter_formula, sizeof(filter_formula)-1))
							...
							RulesWithMacro = MacroForCDOnly(filter_formula);	/* returns 1 if one macro is to be executed : this will force ProcBlackList to compute "Lotus Macro" on fully converted MIME to CD document */
						  ...
						  char * Original_Body_Attach)
{
	...
	if (nbrec > 0)
	{
		...
		/* 3.93.43 : remove the ":" between the SubjectHeader & origsubjet */
		sujet[0]='\0';
								  ...
								  char* Original_Body_Attach)
{
	...
	if ( nbrec > 0 )
	{
		...
		sujet[0]='\0';
		if ( strlen( origsubject) )
			sprintf(sujet, "%s %s", SubjectHeader, origsubject);  /* 3.93.43 : remove the ":" between the SubjectHeader & origsubjet */
		else
			sprintf(sujet,"%s", SubjectHeader);
		...
		if (strcmp(Append_Orig_Body,"0"))
		{
			...
			if (Append_Orig_Body[0] == '1')
			{
				...
				if (DocLink == 1)
				{
					...
					erreur = SimpleAppendDocLink(replica_ID, Note_UNID, View_UNID, sujet, phCompound);
				}
			}
			else
			{
 ...
 * 3.93.35	: rename function; append mw_ before
 ********************************************************************/
	...
	while(error==NOERROR)
	{
		...
		if ((wType & TYPE_OBJECT) == TYPE_OBJECT)
		{
			...
			if (pFileObject.Header.ObjectType == OBJECT_FILE)
			{
				...
				if (strlen(statmw[lgstat].Ext) + strlen(loc) + 2 > 145)
					strcat(statmw[lgstat].Ext, "...");
				else
				{
					if (strlen(statmw[lgstat].Ext))
					{
					 	strcat(statmw[lgstat].Ext, ", ");
...
void dumpstat(void)
{
	...
	{
	  sprintf(temp, "%s;%s;%s;%s;%s;%s;%d;%s;%s;%lu;%d;%s;%d;%d;%d;%d;%d;%d;%lu;%d;%d;%d;%s;%s;%s;%s",
							statmw[i].Date,
							statmw[i].Time,
							statmw[i].PostedDate,
							statmw[i].PostedTime,
...
STATUS SimpleAppendDocLink(TIMEDATE replica_ID, UNID Note_UNID, UNID View_UNID, char* DocLinkText, HANDLE hCompound)
{
	...
	/* 3.93.39, 3.93.40 : URL formatting */
	/* sprintf(URLText, "<HTML><TITLE></TITLE><HEADER></HEADER><BODY><a href=\"http://%s/%s/%s/%s?opendocument\"> web : %s </a></BODY></HTML>", mail_server_abbr, DBUniqueID, ViewUniqueID, DocUniqueID, DocLinkText );*/
...
* ---------------------
* 29/12/04	: Revision 3.93.31
*			: return(1) if fieldname is not available.
*********************************************************************************/
STATUS MakeStringFromNotesAddressList(NOTEHANDLE hNote, char* fieldname, char* out_string, int max_len)
{
	STATUS	error = NOERROR;
	...
	/* 3.93.31  : exit with error if field is not available */
	if (!NSFItemIsPresent(hNote, fieldname, (WORD) strlen( fieldname)))
		return(1);
...
STATUS  AppendReaderItem (NOTEHANDLE  hNote, char* fieldname, char* szReader, int check)
{
	...
	if (check)
	{
		...
		if ( GetUserAddressFullName(szReader, szReader, "fullname") )
		{
...
* OUTPUT	: List* point to all addresses foundd
***************************************************************************************/
			...
			if ( P != NULL )	/* 3.93.8 */
			{
				...
				if ( username != NULL )	/* 3.93.8 */
				{
					...
					*/
					...
					/* 3.93.27 : TRUE -> FALSE; not to allow duplicate entries */
					NSFItemAppendTextList(hNote, MAILNOTE_RECIPIENTS_FIELD, username, (WORD) strlen(username), FALSE);
...
* OUTPUT		: STATUS (error / NOERROR).
*********************************************************************************************/
STATUS GetDominoHostName(DBHANDLE dbnab, char * capath, char* ServerName, char* Serverhostname)
{
	STATUS				error = NOERROR;
	...
	if (error = NIFFindDesignNote(dbnab,
								  ...
								  &nab_server_view_id))
	{
		AddInLogMessageText(ERR_FIND_VIEW, error, NOMAPI, NAB_SERVER_VIEW, capath );
		return(error);
	}

	/* Open view  */
	if (error = NIFOpenCollection(dbnab, dbnab,
								  nab_server_view_id,
								  OPEN_REBUILD_INDEX,NULLHANDLE,
								  &server_view_coll,
								  NULL, NULL, NULL, NULL))
	{
	    AddInLogMessageText(ERR_OPEN_VIEW, error, NOMAPI,  NAB_SERVER_VIEW, capath );
		return(error);
	}
							  ...
							  &atraiter))
	{
		...
		AddInLogMessageText(ERR_FIND_CONFIG, error, NOMAPI, ServerName, NAB_SERVER_VIEW, capath );
		return(error);
	}
							   ...
							   &buffer_cahandle, NULL, NULL, &atraiter, NULL))
	{
		...
		NIFCloseCollection(server_view_coll);
		AddInLogMessageText(ERR_READ_CONFIG, error, NOMAPI, ServerName,  NAB_SERVER_VIEW, capath );
		return(error);
	}
