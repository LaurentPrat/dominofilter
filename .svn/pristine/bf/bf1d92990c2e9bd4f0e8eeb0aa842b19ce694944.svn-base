 ...
 * -----------------------------------------------------------------------------------------------------------------
 * 03/05/06				: revision 3.94.50.
 *						: fix in : NSFGetAttachmentFileName(...., WORD MaxSize);
 *						: fix in : mw_NSFNoteGetSize()
 * 05/03/06				: revision 3.94.51
 *						: change in GetFieldInList - new argt : MAXList
 * 31/05/06				: revision 3.94.54.
 *						: procblacklist: "Importance" is not correct use "DeliveryPriority" instead.
 * 16/06/06				: revision 3.94.55
 *						: INTEGRATION OF revision 3.93.45
 *						: change in OverQuota : change FROM/PRINCIPAL for overquota test.
 * 16/10/06				: revision 3.94.56.
 *						: Keep the MW_MACRO_FIELD_NB_RECIPIENTS field wirhin distributed message
 * 14/11/06				: revision 3.94.57-59
 *					    : Mark Notification with DocLink for MWSARC - $MessageID
 *						: NSFItemSetText(hMes, "MWDOCLINK", "1", 1);
 *						:
 * 16/11/06				: revision 3.94.60
 *						: NIFOpenCollection (4th argt - set to ZERO all the time )
 *						:
 * 19/01/07				: revision 3.94.61
 *						: Integrate the 3.95.15 fix in RemoveNotesDomain()
 *						: revision 3.94.62.
 *						: fix in GetBlackListRules :
 *						: rulesw       = abs((int) RulesPriority);
 *						:
 * 05/03/07				: revision 3.94.63
 *						: Fix : OpenDbs ( cf 3.93.46 )
 *						:
 * 15/03/07				: revision 3.94.64 / fix regression caused by OpenDbs (3.93.46)
 *						:
 * 25/03/07				: revision 3.94.65 : Fix : ScanMail AIX - Exclude Task issues.
 ...
 *						: revision 3.94.66 : if ( NSFItemIsPresent(hNote, "Mailbox_AppLock", 15))
 *						:
 * 20/07/07				: revision 3.94.67 : compatibility with HPRIM_API notes.ini env :
 *						: Changes in OpenDbs.
 *						: add notes.ini option : MailWatcherTask_NotExpandGroups
 *						:
 * 24/07/07				: revision 3.94.68 - QA
 *						: Support HPRIM_API; env config for main API db.
 *						:
 * 29/08/07				: revision 3.94.69
 *						: Read 'MailWatcherTask_NotExpandGroups' from mail_watcher in mw_api.nsf
 *						:
 * 						: revision 3.94.70 : Comment this warning
 *						: AddInLogMessageText(ERR_OPEN_NOTE, sErreur, NOMAPI, note_id, "mail.box");
 *						:
 * 20/09/07				: revision 3.94.71
 *						: DO not validate the process if a special field is there ( MW_KEEP_MSG_HOLD )
 *						: new Sub into ProcBlackList : MailInForJournaling()
 *						:
 * 03/12/07				: revision 3.94.72
 *						: Check error into : AppendDocLink;  MailCopyGetDocLinkDetail
 *						:
 *						: revision 3.94.73
 *						: Count number of mail.box - do not try to open non existing mail.box
 *						: Fix. QA
 *						:
 * 25/02/2008			: revision 3.94.74
 *						: Fix in ProcBlackList
 *						: Error handling in 'MailInForJournaling '.
 *						: Add the missing closing of HNotefiltre & Buffer allocated.
 *						:
 *						: revision 3.94.75
 *						: OverQuota Managment.
 *						: FROM vs PRINCIPAL
 *						: requires : mw_api_408.ntf
 *						: Read if OVERQUOTA is FROM or NSF Based ( if NSF based then 'Principal' overwrites 'From' )
 *						: Default : advquotafrom = FALSE;
 *						:
 * 19/03/2008			: revision 3.94.76
 *						: ADD replicaID into the Notification created with DOCLINK references.
 *						: change in : SimpleProcCrochet_DocLink()
 * ----------------------------------------------------------------------------------------------------------------
 * 31/03/2008			: MAILWATCHER VERSION GOES V4. - Keep build # (76 + 1)
 *						: revision 4.0.77
 *						: Extend Debug Mode  
 *						: Use the ( MWBLACKLISTDEBUG ) in ProcBlackList.
 *						: FROM/TO/FORMULA for Match + Except + Rules Memory conditions.
 * 02/04/2008			: revision 4.0.78
 *						: fix in Except_Match : Add ' to Verbose print out - Except_To.
 *						: revision 4.0.79
 *						: fix in ProcBlackList : Verbose mode
 *						: 
 * 14/04/2008			: revision 4.0.80
 *						: Fix 'Mono mail.box detected' vs 'Multi mail.box detected'
 *						: Fix Error warning in ProcBlackList() for 'ERR_OPEN_FILTER_DOC'
 *						:
 * 13/05/2008			: revision 4.0.81
 *						: Fix 'MailInForJournaling'
 *						: FROM != Recipient - when FROM == Recipients and FROM is Group 
 *						: then router do not deliver the mail.
 *						: if FROM is to be overflow by addin a prefix then GV 'alertfrom' is used.
 *						: 'alertfrom' is defined into 'mail_watcher' document.
 * 23/06/2008			: 4.0.82
 *						: use "NOHEADER" into ForwardTo - NonDelivery* - to skip HEADER construction into notification mail
 *						: 4.0.83
 *						: Extend Verbose . CCR issue. 8.0.1. - infinite loop.
 *						: DOMINO BUG on NSFNoteOpen - TODO : use a TIMEOUT.
 * 26/06/2008			: 4.0.84
 *						: QTA Mgt. FROM vs PRINCIPAL.
 *						: keep Principal unchanged if FROM is not always use as Mail Owner for QTA control
 * 06/10/2008			: revision 4.0.85
 *						: Move Verbose line -  display only when some documents have been found.
 * 23/10/2008			: revision 4.0.86
 *						: MIME to CD conversion can loose $FILE.
 *						: Add a field : MW_HAS_ATTACHMENT_MARKER.
 * 05/11/2008			: revision 4.0.87
 *						: Journaling rules : KeepFromField=1 - notes.ini
 *						: Prevent from changing From field in MailInForJournaling()
 * 07/11/2008			: revision 4.0.88
 *						: use '$JournalResponsibility="2"' to in MailInForJournaling()
 *						: to prevent email from boucing to original sender.
 * 07/11/2008			: revision 4.0.89
 *						: replace OPEN_REBUILD_INDEX when it is not necessary to recreate the index : OPEN_NOUPDATE
 *						: revision 4.0.90
 *						: WORK AROUND AIX 64 - NIFUPDATECOLLECTION BUG
 *						: add a conditional flag to not to "OPEN_REBUILD_INDEX"
 *						: set config MWREFRESHVIEWMODE=NEVER
 * 10/11/2008			: revision 4.0.91
 *						: restore 'OPEN_REBUILD_INDEX' for ($APIVIEW).
 *						: remove remamoing JRN_FROM for MailInForJournaling()
 * 10/07/2009			: revision 4.0.92
 *						: <TODO> : stop calling parseSMTPaddress - it does not support "abc"<abc.test.com>@domainNotes
 *						: because it removes the Notes Domain
 * 07/08/2009			: revision 4.0.93
 *						: upgrade 'STATUS OpenNAB(char* dbPath, char* server_name)'.
 *						: read from notes.ini a remote server for Group Loading withing Filtering Rules.
 *						: notes.ini : [ MWREMOTEFILTERINGDBDIR ]
 *						: TODO : Make sure the REMOTE server is used by NAMELOOKUP(Remoteserver != NULL )
 * 11/08/2009			: revision 4.0.94
 *						: Support DIRECTORY ASSISTANCE for Filter Rules Groups parameters
 *						: new 'BOOL DA_IsaGroupInNab(char* GroupName)'
 *						: new 'void DA_GetUserInGroup(char* GroupName, GrpList** GrpHistory)'
 *						: revision 4.0.1.95
 *						: Build #, 4.0.1 - use Directory Assistance
 *						: UseDirectoryAssistance = FALSE; ( default )
 *						: use notes.ini - [ MWADVSRVDIRECTORYASSISTANCE = 1 ]
 *						: revision 4.0.1.96
 *						: fix in 'GetBlackRedListRules'
 *						: revision 4.0.1.97
 *						: QA & Fix in 'DA_IsGroup..'
 * 12/08/2009			: revision 4.0.1.98
 *						: fix ERROR output when searching for a group.
 *						: revision 4.0.1.99
 *						: UpperCase Group/user before inserting into list ( reg due to 4.0.94 ->  )
 *						: revision 4.0.1.100
 *						: QA/fix List header for users ( reg : 4.0.1.99 )
 * 08/01/2010			: revision 4.0.1.101
 *						: Regression caused by 4.0.92 - ParseSMTPAddress is required to be able to compare
 *						: user's Internet address with all possible forms.
 *						: fix in ProcBlackList()
 *						: to be done on 'Recipients' as the FROM item is already simplified if it is 'SMTPOriginator'
 *						:
 * 28/07/2010			: revision 4.0.1.102
 *						: add '$JournalEmail_ExpandGroups_x' proc for Journaling Rules.
 *						: TODO : field must be NON SUMMARY
 * 29/07/2010			: revision 4.0.1.103
 *						: Create '$JournalEmail_ExpandGroups_x' only if at least one Group has been expanded
 * 30/07/2010			: revision 4.0.1.104
 *						: Create field 'NON SUMMARY' for the first item.
 *						: revision 4.0.1.105
 *						: Fix displayed release/version
 *						: Delete temporary field 'MW_RECIPIENTS' -> #define MW_MACRO_FIELD_RECIPIENTS
 * 30/08/2010			: revision 4.0.1.106
 *						: Extend Log for DEBUG - print out when there is no document to process 
 *						: or no new document since last open.
 * 13/09/2010			: revision 4.0.1.107
 *						: QC9201 : Error when saving the Journal copy - Field is to large, issue with too many SUMMARIES field
 *						: fix : new error output for SAVING != CREATING
 *						: new : add sub to duplicate TEXT LIST to a NON SUMMARY Text List.
 * 11/02/2011			: revision 4.0.1.108
 *						: fix in 'RulesMemoryRestoreFromFile'
 *						: protect from overflow during fscanf.
 * 18/04/2011			: revision 4.0.1.109
 *						: Extend verbose output for filtering rules.
 *						: Total, Frequency, Modulo
 *						: Fix GetFrequency to limit the duration based on the interval
 *						: Fix in ProcBlackList. Apply Rules memory on every FROM*TO, and not only one time per email
 *						: need an extra argument in rules to support previous configuration 
 *						: field : AlwaysApplyMemoryRules = 1;
 *						: TODO/FIX : RulesMemoryDumpToFile, has_run (long) does not get saved correctly
 * 20/04/2011			: revision 4.0.1.110
 *						: Fix RulesMemoryDumpToFile
 *						: Add 'Last_time' for memory rules to adjust interval and frequency calculation
 *						:
 * 20/04/2011			: revision 4.0.1.111 ( TO DO )
 *						: fix in 'NonDeliveryBlackList'
 *						: fix in 'NonDeliveryBlackList_DocLink'
 *						: Prevent the ":" to be appended to the subject if it is null
 *						: Extend verbose for the 'NOHEADER' attributs
 *						:
 * 28/04/2011			: revision 4.0.1.112
 *						: new notes.ini switch to support '$JournalRecipients'
 *						: field 'BlackList_Recipient_Field', check flag 'UseOtherRecipientfield'

 * 02/05/2011			: revision 4.0.1.113 - implement 4.0.1.111
 *						:
 * 03/05/2011			: revision 4.0.1.114 
 *						: Add special config to override Recipients value even if 
 *						: the flag 'UseOtherRecipientfield' flag is active
 * 03/05/2011			: x64 compilation
 *						:
 * 10/05/2011			: Fix based on 4.0.1.115 - Group Expansion must be based on the alternate field name
 *						: if configured.
 *						: Fix in CheckRecipientField; SplitMail
 *						:
 * 02/09/2011			: revision 4.0.1.116
 *						: Extend debug on show task, get the number of doc to be processed
 *						: After the GetLastModified.
 *
 *						: revision 4.0.1.118
 *						: Extend verbose during init phase
 *						:
 * 13/04/2012			: revision 4.0.1.119
 *						: Add the list of BlackListed TO(i) to the NonDeliveryBlackList subs
 *						: NonDeliveryBlackList_DocLink
 *						: use : ProcBl_AppendFieldDetailsToEmailBody(hNoteFiltre, *hNote, "Field_Titles", "Field_Values");
 *						:
 * 22/04/2012			: revision 4.0.1.120
 *						: Add field value in TEXT NOTIFICATION appended at the end of the originale email
 *						: called in AddMessageToEmail()
 *						: new : STATUS GetMacroAndReplace()
 *						:
 * 23/04/2012			: revision 4.0.1.121
 *						: Macro "TO" => ( Recipients(i) ) is aliased as {MW_TO}
 *						: char* safe_recipient -> copied into hNotes as MW_TO.
 *						: Force the Return To Sender to send a copy if configured. 
 *						: Requires : mw_rules_404.ntf
 *						:
 * 23/04/2012			: revision 4.0.1.122
 *						: Extend list of to be removed fields.
 *						:
 * 02/10/2012			: revision 4.1.1.123
 *						: Integrate 3.95.53 / Collect Extension Size for Statistics 'DumpStat'
 *						: collect attachment size after the document size. (use ,) as separator.
 *						: change in mw_NSFNoteGetSize : add ExtSize field.
 *						: revision 4.1.1.124
 *						: fix in mw_NSFNoteGetSize; check malloc()
 *						:
 * 15/10/2012			: revision 4.1.1.125
 *						: changes in 'NonDeliveryBlackList'
 *						: changes in 'NonDeliveryBlackList_DocLink'
 *						: new GetAddressFieldFor()
 *						: Tranfert the addresses into SendTo, CopyTo, BlindCopyTo accordingly.
 *						: fix 'NOHEADER' when more than one recipients
 *						:
 * 11/12/2012			: revision 4.1.1.126
 *						: changes in 'NonDeliveryBlackList'
 *						: changes in 'NonDeliveryBlackList_DocLink'
 *						: for addresses field : FROM,TO, extend the use of the syntax {xxxx},{abcd},OtherValue
 *						: to set dynamically the parameters.
 *						: new : AppendToTextListWithStringAndSeparator()
 *						: because of datatype changes for field :
 *						: BLIST_DF_TO_FWD, BLIST_DF_TO names -> Text. Requires new template for Rules Db
 *						: MW_RULES_405.NTF
 *						: new to remove fields BLIST_DF_TO_FWD, BLIST_DF_TO before updating the rules
 *						: as the backend will not update the data type and not allow ',' as input
 *						:
 * 17/12/2012			: revision 4.1.1.127
 *						: AppendTextListToBodyUsingFieldArgument()
 *						: Extend length
 *						:
 * 08/02/2012			: revision 4.1.1.128
 *						: Fix in MakeStringFromField() - RFC_TEXT is 1282 -> TYPE_RFC822_TEXT
 *						: requires NotesAPI_65 include.
 *						:
 * 03/12/2014			: revision 4.1.1.129
 *						: new : timeslot.c/h
 *						: ProcBlackList, can skip a rules if does not match the BL_TimeSlot
 *
 * 23/04/2015			: revision 4.1.1.130
 *						: Extend Debug verbose & win32 config
 * --[ TO DO ]------------------------------------------------------------------------------------------------------
 *
 * (1) Each module should leave a list of field to be deleted when the last MWTAG is removed!
 *
 * (2) Side note to Laurent:
 * we really need to have a better way of detecting this.
 * We should read the router configuration the same way that the router does.
 * Please put that on your list for a change in a future version,.
 ***********************************************************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int			ProcBlackList(NOTEHANDLE *, DBHANDLE,char* mail_ID, RulesList* L_rules, RulesList** StartL, DBHANDLE, BOOL UseOtherRecipientfield, BOOL KeepOriginalRecipientfieldAsRealRecipient);  /* 4.0.1.112 */
STATUS		GetBlackRedListRules(HCOLLECTION coll_bl, DBHANDLE, BOOL UseDirectoryAssistance);  /* 4.0.1.95 */
...
void		AddMessageToEmail(NOTEHANDLE fNote, NOTEHANDLE* hNote, char* fieldName, DBHANDLE, int*, NOTEHANDLE*, NOTEHANDLE, int, NOTEHANDLE);
...
STATUS		OpenNAB(char *, char*);					/* 4.0.93 */
STATUS		CloseNAB();
...
int			CheckTimeSlotRules(NOTEHANDLE hNote);			/* 4.1.1.129 */
...
STATUS		GetUserAllAlias(char* username, GrpList* GrpHistory);
void		ResetRulesw(int*, int From_User, int To_User);
int			GetUserAlias(char* username, char* fieldname, GrpList* GrpHistory);
...
/* 3.92a */
STATUS		SplitMail(DBHANDLE* hMB, NOTEHANDLE hNote, NOTEHANDLE* hCopy, char* FieldRecipientName);   /* 4.0.1.115 */
...
STATUS		GetAllAddressesFromField(NOTEHANDLE hNote, char* FieldName, char* FromValue, WORD* nbUserFound, List UserAddresses, 
									 Position P, int recurse, List GrpAddresses, Position G);	/* 4.0.1.102 */
...
void		NSFGetAttachmentFileName(BLOCKID bidv, char* FileName, char* FileExtension, WORD);
...
STATUS		NSFItemDuplicateTextListNonSummary(NOTEHANDLE hNote, char* Field_in, char* Field_out, BOOL AllowDuplicateItem);	/* 4.0.1.107 */


/* 3.94 */
int			CheckBlackListRulesMemoryValidation(NOTEHANDLE hNote, RulesList* L_rules, char* RulesRef, int CheckBlackListRulesMemoryValidation );  /* 4.0.1.109 */
...
/* 4.0.94 */
BOOL		DA_IsaGroupInNab(char* GroupName);
void		DA_GetUserInGroup(char* GroupName, GrpList** GrpHistory);

/* 4.0.1.119 */
STATUS		CopyFieldListFromOneNoteToAnother(NOTEHANDLE hNoteFiltre, char* sFieldRef, NOTEHANDLE horig_notes, NOTEHANDLE hMes);

/* 4.0.1.120 */
STATUS		GetMacroAndReplace(NOTEHANDLE hNote, char* in_string, char* out_string, int max_len);
STATUS		ReplaceMacroFieldinText(NOTEHANDLE hNote, char* fieldname, char* in_string, char* out_string, int max_len);
STATUS		MakeStringFromField(NOTEHANDLE hNote, char* fieldname, char* out_string, int max_len);

/* 4.0.1.125 */
STATUS		GetAddressFieldFor(NOTEHANDLE horig_note, char* sendTobis, char* AddressField);

/* 4.1.1.126 */
STATUS	AppendToTextListWithStringAndSeparator(NOTEHANDLE hNote, char * fieldName, char* fieldValues);
...
int			pasint;
int			statint;
DBHANDLE	hDb							= NULLHANDLE; /* MW_SETUP.NSF handle */
DBHANDLE	hDbRules					= NULLHANDLE;	/* MW_SETUP or MW_RULES */
static		HCOLLECTION coll_bl			= NULLHANDLE;
static		HCOLLECTION coll_bl_formula	= NULLHANDLE;
static		HCOLLECTION coll_cr			= NULLHANDLE;
static		HCOLLECTION coll_bs			= NULLHANDLE;
static		HCOLLECTION coll_quota		= NULLHANDLE; /* view : (POLICES) -> 3.9 */

struct		advt_stat statmw[MAXLGSTAT];
int			lgstat;
char		StatFile[MAXPATH];
BOOL		advstat, advstat_subject, advarch, advblck, advform, advnotify_blacklist,advblck_rm, advquotafrom;
...
char		dbnab_server[MAXPATH];	/* 4.0.93 : remote server for address book */
char		TempAddressConcatSendTo[MAX_DOC_FIELD_TEXT_SIZE];		/* 4.0.1.125 */
char		TempAddressConcatCopyTo[MAX_DOC_FIELD_TEXT_SIZE];		/* 4.0.1.125 */
char		TempAddressConcatBlindCopyTo[MAX_DOC_FIELD_TEXT_SIZE];	/* 4.0.1.125 */
...
BOOL	MailWatcherTask_NotExpandGroups = FALSE;				/* AntiVirus issue - AIX SMDREAL 3.94.67 */
char	BlackList_Recipient_Field[100];							/* 4.0.1.112 */
...
STATUS far PASCAL  AddInMain (HMODULE hModule, int argc, char *argv[])
{
	...
	int				MaxCountMailBox = 1;			/* 3.94.73 */
	int				hasAttachment = 0;				/* 4.0.86  */
	BOOL			UseDirectoryAssistance = FALSE;	/* 4.0.1.95 */
	BOOL			UseOtherRecipientfield = FALSE; /* 4.0.1.112 */
	BOOL			KeepOriginalRecipientfieldAsRealRecipient = FALSE; /* 4.0.1.112 */
	DWORD			num_ID_Table_entries = 0;		/* 4.0.1.116 */


	/* Init - check out if a task is already running */
	advquotafrom = FALSE;		/* 3.94.75 */
	MWDebug = FALSE;
	...
	if (OSGetEnvironmentString("MWBLACKLISTDEBUG", szDomain, sizeof(szDomain)-1))		/* 4.0.83 - spelling */
	...
	/* 4.0.1.95 : Directory Assistance - via notes.ini */
	if ( OSGetEnvironmentString("MWADVSRVDIRECTORYASSISTANCE", szDomain, sizeof(szDomain)-1))
	{
		if ( szDomain[0] == '1' )
			UseDirectoryAssistance = TRUE;
	}
	...
	/* 4.0.93 : Remote Server for remote dbnab ( dbnab_server ) */
	OSGetEnvironmentString("MWREMOTEFILTERINGDBDIR", dbnab_server, sizeof(dbnab_server)-1);


	/* 4.0.1.112 : check for BlackList "other" recipient field  : BlackList_Recipient_Field[ 100 ] */
	if ( OSGetEnvironmentString("MWBLACKLISTRECIPIENTSFIELD", BlackList_Recipient_Field, sizeof(BlackList_Recipient_Field)-1) )
	{
		if ( strlen(BlackList_Recipient_Field) )
			UseOtherRecipientfield = TRUE;

		if ( MWBLACKLISTDEBUG & UseOtherRecipientfield )
			AddInLogMessageText(API_BLIST_OTHER_REC_FIELD, NOERROR, NOMAPI, BlackList_Recipient_Field);

		/* 4.0.1.114 */
		if ( UseOtherRecipientfield )  /* KeepOriginalRecipientfieldAsRealRecipient */
		{
			if ( OSGetEnvironmentString("MWBLACKLISTRECIPIENTSFIELDKEEPORIGINAL", szDomain, sizeof(szDomain)-1) )
			{
				if ( szDomain[0] == '1' )
					KeepOriginalRecipientfieldAsRealRecipient = TRUE;

				if ( MWBLACKLISTDEBUG & KeepOriginalRecipientfieldAsRealRecipient )
					AddInLogMessageText(API_BLIST_KEEP_ORIG_REC_FIELD, NOERROR, NOMAPI);
			}
		}
	}
	...
	if (sErreur = OpenDbs())
	{
		...
		LogErr("Can not init : OpenDbs()", sErreur);
		return(NOERROR);
	}

	/* 4.0.1.118 : Extend verbose during init phase */
	AddInSetStatusText(DO_INIT_OPEN_DB);
	...
	if (advblck)
	{
		...
		if( sErreur = OpenNAB(dbnab, dbnab_server))
		{
			AlerteAdmin(4);
			return(sErreur);
		}
		...
		if (advblck) /* advblck is reset to FALSE if an error has occured during OpenNAB */
		{
			...
			{
				/* 4.0.1.118 : Extend verbose during init phase */
				AddInSetStatusText(DO_INIT_LOADING_RULES_MEM);
				...
				/* 4.0.1.118 : Extend verbose during init phase */
				AddInSetStatusText(DO_INIT_LOADING_RULES_MEM_LOAD);
			}
		}

		/* 4.0.1.118 : Extend verbose during init phase */
		AddInSetStatusText(DO_INIT_RULES_LOADED);
	}
...
/*******************************************************************************************/
		...
		if (AddInMinutesHaveElapsed((DWORD) pasint))
		{
			...
			while ( (!sErreur) && ctr < MaxCountMailBox )			/* 3.94.73 */
			{
				/* Check mail(x).Box */
				ctr++;
				...
				{
					/* 4.0.1.106 :  Extend LOG/Debug  */
					if ( MWBLACKLISTDEBUG == TRUE )
						AddInLogMessageText("MWADVT - 'ERR_NO_MODIFIED_NOTES' : no document to be processed '%s'", NOERROR, filename );
				...
				if (sErreur)
				{
					/* 3.94.73 : Set the real number of mail.box found */
					if ( ctr )
					{
						MaxCountMailBox = (ctr -1);
						if ( MaxCountMailBox == 0 )
							AddInLogMessageText("%s : Mono mail.box detected", NOERROR, NOMAPI);
						else
							AddInLogMessageText("%s : Multi mail.box [%d] detected", NOERROR, NOMAPI, MaxCountMailBox);
					}
				...
				/* 4.0.83 - .85: move verbose here : Extend LOG/Debug */
				if ( MWBLACKLISTDEBUG == TRUE )
					AddInLogMessageText("MWADVT - now processing documents into '%s'", NOERROR, filename );
				...
				/* 4.0.1.116 */
				num_ID_Table_entries = 0;
				
				/* 4.0.1.116 : Expose number of entries */ 
				if ( MWBLACKLISTDEBUG == TRUE )
				{
					num_ID_Table_entries = IDEntries(idtable);
					AddInLogMessageText("MWADVT - now processing %lu documents into '%s'", NOERROR, num_ID_Table_entries, filename );
					sprintf(LogString,"MWADVT - now processing %lu documents into '%s'", num_ID_Table_entries, filename);
					AddInSetStatusText(LogString);
				}
                    ...
                    if (IDIsPresent(idtable, note_id))
					{
						...
						*/
						...
						if (NSFItemIsPresent(hNote, "MB_OtherServices", 16))
						{
							/* 4.0.1.106 :  Extend LOG/Debug  */
							if ( MWBLACKLISTDEBUG == TRUE )
								AddInLogMessageText("MWADVT - 'MB_OtherServices' : skiping document '%s'", NOERROR, filename );
						...
						if ( NSFItemIsPresent(hNote, "Mailbox_AppLock", 15))
						{
							...
							/* 4.0.1.106 :  Extend LOG/Debug  */
							if ( MWBLACKLISTDEBUG == TRUE )
								AddInLogMessageText("MWADVT - 'Mailbox_AppLock' : skiping document '%s'", NOERROR, filename );
						...
						if ( QuotaEnd == 0 )
						{
							...
							/* 4.0.1.102 : Save the current 'Recipients' field into 'MW_Recipients' */
							NSFItemDuplicateTextList(hNote, MAILNOTE_RECIPIENTS_FIELD, MW_MACRO_FIELD_RECIPIENTS, FALSE);
							
							/******************** start getting message stats **********************/
							if (advstat)
							{
								/* 4.0.86 - check for $File */
								if ( NSFItemIsPresent(hNote, "$FILE", (WORD) strlen("$FILE")) )
								{
									hasAttachment = 1;
								}
								...
								if ( NSFItemIsPresent(hNote, MW_MAILDUPLICATE, (WORD) strlen(MW_MAILDUPLICATE) ) == NULL )
								{
									...
									/* 4.0.86 */
									if ( ( pj == 0 ) && ( hasAttachment == 1 ) )
									{
										AddInLogMessageText(ERR_ATTACH_NOT_FOUND, NOERROR, NOMAPI, note_id, filename);
										NSFItemSetText(hNote, MW_HAS_ATTACHMENT_MAKER, "1", 1);
									}
							...
							else
							{
								...
								wnb = NSFItemGetTextListEntries(hNote, MAILNOTE_RECIPIENTS_FIELD);
							...
							/* 3.93.13 */
							/* 3.94.67 : This operation can be skipped - notes.ini config */
							if ( MailWatcherTask_NotExpandGroups == FALSE )
							{
								if ( wnb == 0 )
									CheckRecipientField(hMb, hNote, "SendTo", &wnb, FromValue_abbr);
								else
								{
									if ( UseOtherRecipientfield )
										CheckRecipientField(hMb, hNote, BlackList_Recipient_Field, &wnb, FromValue_abbr);
									else
							...
							{
								/** if Returns 1 => Delete **/
								if (ProcBlackList(&hNote, hMb, statmw[lgstat].DocID, L_rules, StartL, hDbRules, UseOtherRecipientfield, KeepOriginalRecipientfieldAsRealRecipient ) == 1)
								{
...
/* #endif */			/* } */				
						...
						if (MW_SKIPRULE[0] == '\0' || MW_SKIPRULE[0] == '2')
						{
							...
							/* 3.94.71 : DO not validate the process if a special field is there */
							if ( !NSFItemIsPresent(hNote, MW_KEEP_MSG_HOLD, (WORD) strlen(MW_KEEP_MSG_HOLD) ) && ( mwtag > 0 ) )
							{
							...
							/* remove tempo fields */
							NSFItemDelete(hNote, "MW_TO",                            5);
							NSFItemDelete(hNote, "MW_Temp_Field_to_Print",           22);
							...
							/* NSFItemDelete(hNote, MW_MACRO_FIELD_NB_RECIPIENTS,(WORD) strlen(MW_MACRO_FIELD_NB_RECIPIENTS)); */ /* 3.94.46 - 3.94.56 : do not purge this field*/
							NSFItemDelete(hNote, MW_MACRO_FIELD_RECIPIENTS,   (WORD) strlen(MW_MACRO_FIELD_RECIPIENTS));		/* 4.0.1.105 */
						...
						/* 4.0.1.116 - Decrease the ID count */
						if ( MWBLACKLISTDEBUG == TRUE )
						{
							num_ID_Table_entries = num_ID_Table_entries - 1;
							sprintf(LogString,"MWADVT - now processing %lu documents into '%s'", num_ID_Table_entries, filename);
							AddInSetStatusText(LogString);
						}
...
STATUS OpenDbs(void)
{
	...
	if (!OSGetEnvironmentString("UNIPOWER_API", capath, sizeof(capath)-1))
	{
		...
		if ( !OSGetEnvironmentString("MWATCHER_API", capath, sizeof(capath)-1) )
			OSGetEnvironmentString("HPRIM_API", capath, sizeof(capath)-1);

		/* 3.94.67 : compatibility with HPRIM_API */
		if ( strlen(capath) == 0 )
	...
	/* 4.0.91 : need to rebuild at open : OPEN_REBUILD_INDEX*/
#ifndef AIX64
	if ( error = NIFOpenCollection(hConfdb, hConfdb, vca_id, OPEN_REBUILD_INDEX, NULLHANDLE, &coll_ca, NULL, NULL, NULL, NULL))
#else
	if ( error = NIFOpenCollection(hConfdb, hConfdb, vca_id, OPEN_NOUPDATE, NULLHANDLE, &coll_ca, NULL, NULL, NULL, NULL))
#endif
	{
	...
	/* 3.94.75 : advform is not supported anymore -force it to FALSE */
	advform = FALSE;

	/* 3.94.75 : Read if OVERQUOTA is FROM or NSF Based ( if NSF based then 'Principal' overwrites 'From' ) */
	NSFItemGetText(hConfNote, "advquotafrom", temp, sizeof(temp)-1);
	if ((temp[0] | 0x20) == '1')					/* adaption of Quota filtering */
		advquotafrom = TRUE;
	else
		advquotafrom = FALSE;
	...
	*/
	...
	if ( advarch || advblck || advform  || advquota )
	{
		...
		if ( advblck )
		{
			...
			/* 4.0.89 : need to update at opening : OPEN_REBUILD_INDEX */
#ifndef AIX64
			if ( error = NIFOpenCollection(hDbRules, hDbRules, viewmb_id, OPEN_REBUILD_INDEX, NULLHANDLE, &coll_bl, NULL, NULL, NULL, NULL) )
#else
			if ( error = NIFOpenCollection(hDbRules, hDbRules, viewmb_id, OPEN_NOUPDATE, NULLHANDLE, &coll_bl, NULL, NULL, NULL, NULL) )
#endif
			{
				NSFNoteClose(hConfNote);
			...
			/* 4.0.89 : need to update at opening : OPEN_REBUILD_INDEX */
#ifndef AIX64
			 if ( error = NIFOpenCollection(hDbRules, hDbRules, viewmb_id, OPEN_REBUILD_INDEX, NULLHANDLE, &coll_bl_formula, NULL, NULL, NULL, NULL) )
#else
			 if ( error = NIFOpenCollection(hDbRules, hDbRules, viewmb_id, OPEN_NOUPDATE, NULLHANDLE, &coll_bl_formula, NULL, NULL, NULL, NULL) )
#endif
			{
					NSFNoteClose(hConfNote);
		...
		if ( advarch )
		{
		  ...
		  /* 4.0.89 : need to update at opening : OPEN_REBUILD_INDEX */
#ifndef AIX64
		  if (error = NIFOpenCollection(hDb, hDb, viewmb_id, OPEN_REBUILD_INDEX, NULLHANDLE, &coll_cr, NULL, NULL, NULL, NULL) )
#else
		  if (error = NIFOpenCollection(hDb, hDb, viewmb_id, OPEN_NOUPDATE, NULLHANDLE, &coll_cr, NULL, NULL, NULL, NULL) )
#endif
		  {
				NSFNoteClose(hConfNote);
		...
		if (advform)
		{
		  ...
		  /* 4.0.89 : need to update at opening : OPEN_REBUILD_INDEX */
#ifndef AIX64
			if ( error = NIFOpenCollection(hDb, hDb, viewmb_id, OPEN_REBUILD_INDEX, NULLHANDLE, &coll_bs, NULL, NULL, NULL, NULL) )
#else
			if ( error = NIFOpenCollection(hDb, hDb, viewmb_id, OPEN_NOUPDATE, NULLHANDLE, &coll_bs, NULL, NULL, NULL, NULL) )
#endif
		  {
				NSFNoteClose(hConfNote);
		...
		if (advquota)
		{
			...
			/* 4.0.89 : need to update at opening : OPEN_REBUILD_INDEX */
#ifndef AIX64
			if ( error = NIFOpenCollection(hDb, hDb, viewmb_id, OPEN_REBUILD_INDEX, NULLHANDLE, &coll_quota, NULL, NULL, NULL, NULL) )
#else
			if ( error = NIFOpenCollection(hDb, hDb, viewmb_id, OPEN_NOUPDATE, NULLHANDLE, &coll_quota, NULL, NULL, NULL, NULL) )
#endif
		    {
				NSFNoteClose(hConfNote);
	...
	/* 3.94.69 : Get : MailWatcherTask_NotExpandGroups parameters */
	MailWatcherTask_NotExpandGroups = FALSE;
	if ( NSFItemGetText(hConfNote, "AdvMailWatcherTaskNotExpandGroup", temp, sizeof(temp)-1))
	{
		if (strcmp(temp, "1") == 0)
			MailWatcherTask_NotExpandGroups = TRUE;
	}
...
* revision  : 3.93.37
* revision  : 3.94.75 : support (advquotafrom ) to not to overwrite FROM by PRINCIPAL
* revision	: 4.0.84 : keep PRINCIPAL is FROM is not always the owner.
*			: when sending back the notification
***********************************************************************************************************/
int OverQuota(NOTEHANDLE* hNote, NOTEHANDLE* hCopy, int* IsNewDoc, DBHANDLE hDB, BOOL advquota_resend)
{
	...
	/* 3.94.75 : do it only if (advquotafrom == FALSE) */
	if ( strlen(OrigPrincipal) && ( advquotafrom == FALSE )  )
	...
	if (QuotaFilter(*hNote, emetteur_overquota))
	{
		...
		/* 3.92.16 : Change only the Principal */
		/* 4.0.84 : update only if FROM is to be used to control the Quota */
		if ( advquotafrom == 1 )
...
int ProcBlackList(NOTEHANDLE *hNote, DBHANDLE hDB, char* mail_ID, RulesList* L_rules, RulesList** StartL, DBHANDLE hDbRules, 
				  BOOL UseOtherRecipientfield, BOOL KeepOriginalRecipientfieldAsRealRecipient)
{
	STATUS sErreur, RouteThruBool, MailCopyBool, MW_RAPP_Bool, MW_RAPP_AGAIN_Bool;
	char OrigSubject[2500];
	...
	int			AlwaysApplyMemoryRules = 1;				/* tmp : 4.0.1.109 */
	char		Router_recipient[MAXUSERNAME];			/* 4.0.1.114 */
	...
	/* 4.0.1.125 */
	TempAddressConcatSendTo[0] = '\0';
	TempAddressConcatCopyTo[0] = '\0';
	TempAddressConcatBlindCopyTo[0] = '\0'; 
	...
	/* balayage des destinataires : 4.0.1.112 */
	if ( UseOtherRecipientfield )
		nbrec = NSFItemGetTextListEntries(*hNote, BlackList_Recipient_Field);
	else
	...
	/* 4.0.83 */
	if ( MWBLACKLISTDEBUG == TRUE )
		AddInLogMessageText("Filtering : FROM '%s' -> NbRec '%d' - '%s'", NOERROR, emetteurabr, nbrec, OrigSubject );
	...
	for (i=0; i<nbrec; i++)
	{
		...
		/* 4.0.1.112 */
		if ( UseOtherRecipientfield )
			NSFItemGetTextListEntry(*hNote, BlackList_Recipient_Field, i, recipient, sizeof(recipient)-1);
		else
			NSFItemGetTextListEntry(*hNote, "Recipients", i, recipient, sizeof(recipient)-1);

		/* 4.0.1.114 */
		if ( KeepOriginalRecipientfieldAsRealRecipient )
			NSFItemGetTextListEntry(*hNote, "Recipients", i, Router_recipient, sizeof(Router_recipient)-1);

		/* Keep a safe copy of the recipient.. with all the route : @domaineA@domaineB */
		/* 4.0.1.114 : use the original recipient field to get a saved copy */
		if ( KeepOriginalRecipientfieldAsRealRecipient )
			strcpy(safe_recipient, Router_recipient);
		else
		...
		/* 4.0.1.101 */
		parseSMTPaddress(recipientabr, recipientabr);
...
/* #ifdef VERBOSE */
			if ( MWBLACKLISTDEBUG == TRUE )
				AddInLogMessageText("Rules Reinit '%d' for To '%s' ", NOERROR, BL_Rules, recipientabr);
/* #endif */
		}
...
/* #ifdef VERBOSE */
				if ( MWBLACKLISTDEBUG == TRUE )
				{
					AddInLogMessageText("-----[   virtual email FROM - TO(%d/%d)   ]------", NOERROR, i+1, nbrec );
					AddInLogMessageText("From '%s' - To '%s' MATCH rule '%d'", NOERROR, emetteurabr, recipientabr, BL_Rules );
					AddInLogMessageText("rule '%d' - F/T/Formula '%s'", NOERROR, BL_Rules, clef );
				}
/* #endif */
				...
				if ( Except_Match(emetteurabr, recipientabr, BlackRedList[BL_Rules -1]) == 0 )
				{
					...
					sErreur = NIFFindByName(coll_bl_formula, clef, FIND_CASE_INSENSITIVE | FIND_ACCENT_INSENSITIVE, &coll_pos, &atraiter);

					if (sErreur)
					{
						/* 4.0.1.114 : Store Safe_Recipient */
						NSFItemAppendTextList(*hNote, "RecOK", safe_recipient, (WORD) strlen(safe_recipient), FALSE);
						AddInLogMessageText(ERR_READ_VIEW, sErreur, NOMAPI, "rules record", dbrules);
...
/* #ifdef VERBOSE */
								if ( MWBLACKLISTDEBUG == TRUE )
									AddInLogMessageText("Skiping :rules '%d' - From : '%s' To '%s' ", NOERROR, BL_Rules,emetteurabr, recipientabr );
/* #endif */
...
/* #ifdef VERBOSE */
							if ( MWBLACKLISTDEBUG == TRUE )
								AddInLogMessageText("Acting : rules '%d' From '%s' To '%s' ", NOERROR, BL_Rules,emetteurabr, recipientabr );
/* #endif */
						}
		...
		/* ======== 4.1.1.129 : [Skip of TimeSlot does not match] =========================== */
						if (CheckTimeSlotRules(hNoteFiltre))
						{
							/* Fix : 29/3/04 : Close the hNoteFiltre */
							NSFNoteClose(hNoteFiltre);
						...
						if  ( (!strstr(BListMemo, RefRules)) || ( AlwaysApplyMemoryRules ) )		/* 4.0.1.109 */
						{
							/* ======================= [ Rules Memory condition - if set it must be evaluated now ] ====*/
							if ( CheckBlackListRulesMemoryValidation(hNoteFiltre, L_rules, RulesMemoryRef, MWBLACKLISTDEBUG ) )
							{
...
/* #endif */
							...
							if ( !(MailDuplicate[0] == '1') )
							{
								...
								MailCopyBool = NSFItemGetText(hNoteFiltre, "MailCopy", MailCopy, sizeof(MailCopy)-1);


								/* 3.94.71 : MailInJournaling - One time per message - not to run over duplicates */
								if ( NSFItemGetText(hNoteFiltre, BLIST_MAILCOPY_MAILIN, FolderName, sizeof(FolderName)-1) )
								{
									if ( error = MailInForJournaling(hDB, *hNote, FolderName) )
									{
										/* Force end to keep email in its original format */
										i = nbrec;
										BL_Rules = MaxRules;

										/* 3.94.74 Fix : (25/2/08) : Close the hNoteFiltre */
										NSFNoteClose(hNoteFiltre);
								...
								if (MailCopyBool)
								{
									...
									NSFItemGetText(hNoteFiltre, BLIST_MAILCOPY_FOLDERNAME, FolderName, sizeof(FolderName)-1);
									...
									if ( NSFItemGetText(hNoteFiltre, "BList_IncludeDocLink", bl_choice, sizeof(bl_choice)-1) )
									{
										...
										if ( DocLink != 0 )
										{
											...
											SimpleProcCrochet_DocLink(hNoteSafe, MailCopy, MW_RAPP_Bool, MW_RAPP_AGAIN_Bool, &DocLink, ViewName, &replica_ID, &Note_UNID,
												&View_UNID, FolderName, RemoveReturnReceipt, Original_MessageID);
										}
										else
						...
						if (ReturnVal == 0)
						{
							...
							if (NSFItemGetText(hNoteFiltre, "BList_MessageAppend", bl_choice, sizeof(bl_choice)-1))
							{
								if (bl_choice[0] =='1')
								{
									...
									as the email might be MIME and we need to append R.Text field */
									...
									/* 4.0.1.121 : Save in tempo field the current Safe_recipient is 'MW_TO' TEXT field  : field to be added to the "to be removed" fields*/
									NSFItemSetText(hNoteSafe, "MW_TO", safe_recipient, (WORD) strlen(safe_recipient));

									/* TEXT LIST : NSFItemAppendTextList(hNoteSafe, "MW_TO", safe_recipient, (WORD) strlen(safe_recipient), FALSE); */
								
									/* Append the text (hNoteFiltre - BList_Message) to the current (hNote) */
									AddMessageToEmail(hNoteFiltre, hNote, "BList_Message", hDB, &bNewNote, &hNewNote, hNewNoteCD, bNewNoteCD, hNoteSafe /* Original email */);
									...
									/* Special ApplyActionOveride */
									ApplyActionInBlock = ApplyAction;
									if ( ApplyAction == 0 )
									{
										if ( NSFItemGetText(hNoteFiltre, "BList_Message_ReturnToSender_For", bl_choice, sizeof(bl_choice)-1))
										{
											if (bl_choice[0] =='1')
												ApplyActionInBlock = 1;
										}
									}

									/* Apply the routing conditions */
									SetRecOK_FromToForward(emetteur, safe_recipient, *hNote, hNoteFiltre, "RecOK",OrigSubject, 0, ApplyActionInBlock, BListMemo, BL_Rules, &bRecOK,hDB);

								} /* end of NSFItemGetText(..."BList_MessageAppend"....) */
								else
									SetRecOK_FromToForward(emetteur, safe_recipient, *hNote, hNoteFiltre, "RecOK", OrigSubject, 0, ApplyAction, BListMemo, BL_Rules, &bRecOK, hDB);
						...
						if (NSFItemGetText(hNoteFiltre, "BList_DFMail", bl_choice, sizeof(bl_choice)-1))
						{
							if (strcmp(bl_choice,"1") == 0)
							{
								...
								/* 4.1.1.125 : do not reset if value is 'NOHEADER' */
								{
									if ( strcmp(NewMemo_Fwd, "NOHEADER") )
										strcpy(NewMemo_Fwd, "NULL");
								}
...
{
		if (bRecOK < 0)
			AddInLogMessageText("RULES : '%d' To '%s' NOT OK '%d'", NOERROR, BL_Rules, safe_recipient, bRecOK);
		else
			AddInLogMessageText("RULES : '%d' To '%s' OK     '%d'", NOERROR, BL_Rules, safe_recipient, bRecOK);
}
/* #endif */
	...
	if ( ReturnVal == 0 )
	{
		/* 3.94.71 : Keep original unchanged */
...
void AddMessageToEmail(NOTEHANDLE fNote, NOTEHANDLE* hNote, char* fieldName, DBHANDLE hDB,
					   ...
					   NOTEHANDLE hNoteSafe /* Original email */)
{
	...
	**/
	...
	char			szMessageTextConverted[MAXTXTMESSAGE + 1024];	/* 4.0.1.120 */
	...
	if (szMessageText[0] == '1')
	{
		...
		if (NSFItemGetText(fNote, "BList_Message_Attachment", filename, sizeof(filename)-1 ))
		{
			...
			if (erreur = NSFNoteAttachFile(*hNote,"$FILE",(WORD) strlen("$FILE"),
                                filename, filename, COMPRESS_NONE))
	...
	else
	{
		...
		if ( !NSFItemIsPresent(*hNote, "$NoteHasNativeMIME", 18) )
		{
			...
			/* 4.0.1.120 : Can support both parameters */
...
/*			{	 */
				...
				if ( nb_Messagetxt > 0 )
				{
					...
					{
						text_len = NSFItemGetTextListEntry(fNote, fieldName, (WORD) i, szMessageText, (WORD) sizeof(szMessageText)-1);
						szMessageText[text_len] = '\0';
						szMessageTextConverted[0]='\0';		/* 4.0.1.120 */

						/* 4.0.1.120 : Extend the TEXT with some macro : {FIELD}/{TO} */
						GetMacroAndReplace(hNoteSafe /* Orig Message */, szMessageText, szMessageTextConverted, sizeof(szMessageTextConverted));
						...
						if (erreur = CompoundTextAddParagraphExt
								 ...
								 (hCompound,
                                  dwNormalStyle,
                                  DEFAULT_FONT_ID,
                                  szMessageTextConverted,
                                  (DWORD)strlen(szMessageTextConverted),
                                  NULLHANDLE))
						{
							CompoundTextDiscard(hCompound);
							return;
							   ...
							   char* Original_MessageID)
{
	...
	DBHANDLE	hPirate = NULLHANDLE;
	NOTEHANDLE	hCopy = NULLHANDLE;
	OID			new_oid;
	int			mwtag = 0;
	char		szDbReplicaID[DOCID];		/* 3.94.76 */
	...
	/* ----------------------- Archiving links -------------------------------------------------------*/
	/* 3.95.12 - 3.94.58 : use by MWSARC like program */
	NSFItemSetText(hCopy, MAIL_ORIGDOCLINK, Original_MessageID, (WORD) strlen(Original_MessageID));

	/* 3.94.76 : ADD replicaID use by MWSARC like program ( argt : TIMEDATE* replica_ID ) */
	sprintf(szDbReplicaID, "%08X%08X", replica_ID->Innards[1], replica_ID->Innards[0]);
	NSFItemSetText(hCopy, MAIL_ORIGDOCLINK_DBID, szDbReplicaID, (WORD) strlen(szDbReplicaID));
...
/* #ifdef VERBOSE */
		...
		/*
		if ( MWBLACKLISTDEBUG == TRUE )
			AddInLogMessageText("Rules key : %s", NOERROR, Key );
		*/
/* #endif */
	}
 ...
 *			: C++ debugger is not happy ?? (Stack corrupted).
 *************************************************************************/
STATUS GetBlackRedListRules(HCOLLECTION coll_bl, DBHANDLE hDb, BOOL UseDirectoryAssistance)
		...
		if (hBuffer != NULLHANDLE)
		{
			  	...
			  	for (i= 0; i< (int) NotesFound; i++)
				{
					...
					else
					{
						...
						rulesw       = abs((int) RulesPriority);		/* 3.94.62 : must be positive */
						...
						switch(rulesw)
						{
							...
							case 11:
								...
								if (From_User)
								{
									...
									if ( GetUserAllAlias(tmpr.From, GrpHistory) )
										From_User = 0;
								}
								...
								if (To_User)
								{
									if ( GetUserAllAlias(tmpr.To, GrpHistory) )
										To_User = 0;
								}
							...
							case 12: case 14:
								...
								/* {*/
									...
									/* 4.0.1.95 : support DA */
									if ( UseDirectoryAssistance )
										DA_GetUserInGroup(tmpr.To, &GrpHistory);  /* modif : 23/4/01 */
									else
										GetUserInGroup(tmpr.To, &GrpHistory);  /* modif : 23/4/01 */
								/*}*/
								break;
							...
							case 21: case 41:
								...
								/*{*/
									...
									/* 4.0.1.95 : support DA */
									if ( UseDirectoryAssistance )
										DA_GetUserInGroup(tmpr.From, &GrpHistory);  /* modif : 23/4/01 */
									else
										GetUserInGroup(tmpr.From, &GrpHistory);
								/*}*/
							...
							case 22: case 24: case 42: case 44:
								...
								/*{*/
									...
									if ( UseDirectoryAssistance )
										...
										GetUserInGroup(tmpr.To, &GrpHistory);
								/*}*/
						...
						switch(rulesw)
						{
							...
							case 11:
								...
								{
									/* if error then From_user is ZERO so the RulesW will not be changed so user will remain user and not Group */
									if ( GetUserAllAlias(tmpr.Except_From, GrpHistory) )
										From_Except_User = 0;
								}
								...
								if (To_Except_User)
								{
									if ( GetUserAllAlias(tmpr.Except_To, GrpHistory) )
										To_Except_User = 0;
								}
							...
							case 12: case 14:
								...
								/*{*/
									...
									/* 4.0.1.95 : support DA */
									if ( UseDirectoryAssistance )
										DA_GetUserInGroup(tmpr.Except_To, &GrpHistory);  /* modif : 23/4/01 */
									else
										GetUserInGroup(tmpr.Except_To, &GrpHistory);
								/*}*/
								break;
							...
							case 32: case 34:
								...
								/*{*/
									...
									/* 4.0.1.95 : support DA */
									if ( UseDirectoryAssistance )
										DA_GetUserInGroup(tmpr.Except_To, &GrpHistory);
									else									
										GetUserInGroup(tmpr.Except_To, &GrpHistory);
								/*}*/
								break;
							...
							case 23: case 43:
								...
								/*{*/
									...
									/* 4.0.1.95 : support DA */
									if ( UseDirectoryAssistance )
										DA_GetUserInGroup(tmpr.Except_From, &GrpHistory);
									else
										GetUserInGroup(tmpr.Except_From, &GrpHistory);
								/*}*/
								break;
...
/* OUTPUT : STATUS                          */
/********************************************/
STATUS OpenNAB(char* dbPath, char* server_name)
{
	...
	/* 4.0.93: allow remote - Build Path if ServerName is not Local */
	if ( strlen(server_name) )
		error = OSPathNetConstruct(NULL, server_name, dbPath, fullpath_name);
	...
	if ( error )
	{
		...
		strcpy(fullpath_name, dbPath);
    

    /* Open the Db */
	if (error = NSFDbOpen(fullpath_name, &hDbNAB))
	{

		AddInLogMessageText(ERR_OPEN_DB, error, NOMAPI, fullpath_name);
		AddInLogMessageText(POLSTOP, error);
	...
	/************* $Group collection **********************/
	/* 4.0.89 : do not need to update at opening : OPEN_REBUILD_INDEX replaced by OPEN_NOUPDATE */
	 if (error = NIFOpenCollection(hDbNAB, hDbNAB, viewmb_id, OPEN_NOUPDATE,
									NULLHANDLE, &coll_grp,
									NULL, NULL, NULL, NULL))
	  {
 ...
 *********************************************************************************/
void GetUserInGroup(char* GroupName, GrpList** GrpHistory)
{
	...
	if ((buffer_fhandle != NULLHANDLE) && (atraiter > 0))
	{
		...
		for (i=0; i<nbrec; i++)
		{
			...
			if (IsaGroupInNab(GroupItem))
			{
				...
				{
					GetUserInGroup(GroupItem, GrpHistory); /* Dump now ! */
				}
			}
			/* Standard user */
			else
			{
				...
				GetUserAllAlias(GroupItem, *GrpHistory);
			}
		} /* end of for */
 ...
 * Read a group, create all the final user entry in the SpecialUSerList
 * using DIRECTORY ASSISTANCE - NAMELOOKUP2 ( 2 : to support larger buffer )
 * 4.0.94
 *********************************************************************************/
void DA_GetUserInGroup(char* GroupName, GrpList** GrpHistory)
{
	...
	DWORD		numret;
	char		GroupItemAbb[MAXUSERNAME];
	char		MemberItem[MAXUSERNAME];
	int			k = 0;

	/* 4.0.1.99 : UpperCase it, no case sensitive */
	UpperCase(GroupName);

	/* Query using DA */
	if ( error = NAMELookup2(NULL, 0, 1, "($Users)", 1, GroupName, 1, "Members", &hLookup) )
	{
	...
	/* Lock buffer - prepare data - exit if nothing */
	pLookup = OSLockObject(hLookup);
	pName = NAMELocateNextName2(pLookup, NULL, &numret);
	if (!numret)
	{
		OSUnlockObject(hLookup);
		OSMemFree(hLookup);

		AddInLogMessageText(ERR_READ_GROUP, error, NOMAPI, GroupName, "DIRECTORY ASSISTANCE");	/* 4.0.1.98 :  Proper errors msg for group not found */
		/* Remove the current group so the last (but in the fact the first of the pile) */
		PopGroupHistory(GrpHistory);
		return;
	}
	...
	/* Read buffer */	
	pMatch = NAMELocateNextMatch2(pLookup, pName, NULL);
	k = 0;
	
	do
	{
		if ( error = NAMEGetTextItem2(pMatch, 0, k, MemberItem, MAXUSERNAME-1) )
		{
			/* Error when k = 0 means there is NO "Member field" => user */
			if ( k == 0 )
			{
				DNAbbreviate(0L, NULL, GroupName, GroupItemAbb, MAXUSERNAME, NULL);
				FirstEle = InsertUser(FirstEle, GroupItemAbb, GroupName, *GrpHistory);
				/* GroupName is a member(user) - let's see if (he/she)has some alias to be uploaded too */
				GetUserAllAlias(GroupName, *GrpHistory);
			}
			break;
		}
		...
		/* Recursive call if "GROUPITEM" is also a group and has not been already Dump !!! */
		if ( DA_IsaGroupInNab(MemberItem) )
		{
			/* 4.0.1.99 : UpperCase it, no case sensitive */
			UpperCase(MemberItem);

			/* This global variable is maintained but is not used anymore as the Local Group Pile is (*GrpHistory) */
			tmpGrpList = DumpGrpList;
			DumpGrpList = InsertGroup(DumpGrpList, MemberItem); /* mark as dump ! */

			tmpGrpList = (*GrpHistory);
			(*GrpHistory) = InsertGroup((*GrpHistory), MemberItem); /* fix : 23/4/01 : Start the Group history required for group membership inheritance */

			if (tmpGrpList != (*GrpHistory))
			{
				DA_GetUserInGroup(MemberItem, GrpHistory); /* Dump now ! */
			}
		}
		/* Standard user */
		else
		{
			if ( strlen(MemberItem) )
			{
				UpperCase(MemberItem);
				DNAbbreviate(0L, NULL, MemberItem, GroupItemAbb, MAXUSERNAME, NULL);
				FirstEle = InsertUser(FirstEle, GroupItemAbb, MemberItem, *GrpHistory);
				GetUserAllAlias(MemberItem, *GrpHistory);
			}
		}		
...
BOOL DA_IsaGroupInNab(char* GroupName)
{
	...
	if ( error = NAMELookup(NULL, 0, 1, "($Users)", 1, GroupName, 1, "Members", &hLookup) )
	{
		if ( NULLHANDLE != hLookup )
			OSMemFree(hLookup);
	...
	do
	{
		if ( error = NAMEGetTextItem(pMatch, 0, k, MemberItem, MAXUSERNAME -1) )
		{
			/* Error when k = 0 means there is NO "Member field" => user */
			if ( k == 0 )
				isGroup = FALSE;
...
/* #ifdef VERBOSE */
	...
	if ( MWBLACKLISTDEBUG == TRUE )
	{
		AddInLogMessageText("From '%s' - Exception_From '%s'  Matching test '%d' ", NOERROR, From, CurrentRules.Except_From, result1);
		AddInLogMessageText("To   '%s' - Exception_To   '%s'  Matching test '%d' ", NOERROR, To, CurrentRules.Except_To,     result2);
		AddInLogMessageText("Exception type '%d'        Final Matching test '%d' ", NOERROR, PatternMatch,                   result);
	}
 ...
 *				:
 * 4.1.1.126	: Support : GetMacroAndReplace(NOTEHANDLE hNote, char* in_string, char* out_string, int max_len)
 *				: if field argument is {xxxx}, then the field value defined by xxxx is used as parameter value
 *****************************************************************************************************************/
						  ...
						  char * Original_Body_Attach)
{
	...
	char			bl_choice[4];		/* 4.0.1.119 */
	char			AddressField[MAXUSERNAME];	/* 4.0.1.125 */
	char			bck_AddressField[MAXUSERNAME];	/* 4.0.1.126 */
	...
	if (strcmp(sendto,"0"))
	{
		/* 4.1.1.126 : horig_note : the mail */
		AddressField[0]='\0';
		GetMacroAndReplace(horig_note, sendto, AddressField, MAXUSERNAME);		

		/* 4.1.1.126 */
		strcpy(bck_AddressField, AddressField); /* Back up, strtok */
		AppendToTextListWithStringAndSeparator(hMes,"SendTo", AddressField);
		AppendToTextListWithStringAndSeparator(hMes,"Recipients", bck_AddressField);

		/*
		NSFItemAppendTextList(hMes, "SendTo", AddressField, (WORD) strlen(AddressField), FALSE);
		NSFItemAppendTextList(hMes, "Recipients", AddressField, (WORD) strlen(AddressField), FALSE);
		*/
	...
	{
		/* 4.0.1.125 : Get the Correct 'SendTo', 'CopyTo', 'BlindCopyTo' and saves it into 'AddressField' field 
		so the notification has the right "SendTo" value in the right field (TO,CC,BCC)                         */
		AddressField[0]='\0';
		GetAddressFieldFor(horig_note, sendtobis, AddressField);
	
		NSFItemAppendTextList(hMes, AddressField,     sendtobis, (WORD) strlen(sendtobis), FALSE);
		NSFItemAppendTextList(hMes, "Recipients",     sendtobis, (WORD) strlen(sendtobis), FALSE);
		nbrec = 1;
	}

	/* if forwardto <> "NULL" => Append */
	if ( strcmp(forwardto, "NULL") && strcmp(forwardto,"NOHEADER") )		/* 4.0.82 */
	{
		/* 4.1.1.126 : Get Field value if {xxx} */
		AddressField[0]='\0';
		GetMacroAndReplace(horig_note, forwardto, AddressField, MAXUSERNAME);	
	...
	if (nbrec > 0)
	{
		...
		{
			/* 4.0.1.113 */
			if ( strlen(SubjectHeader) )		
				sprintf(sujet, "%s : %s", SubjectHeader, origsubject);
			else
				strcpy(sujet, origsubject);
		}
		else
		...
		if (strcmp(Append_Orig_Body,"0"))
		{
			...
			if (Append_Orig_Body[0] == '1')
			{
				...
				if ( strlen( from )  && strcmp( forwardto, "NOHEADER" ) )		/* 4.0.82 : if NULL - disable HEADER generation */ 
				{
					/* Original Header */
		...
		/** 4.0.1.119 : Add FIELD LIST after the message BODY ***/
		if ( NSFItemGetText(hBL_Note, "Notif_MessageAppend_Field_Values", bl_choice, sizeof(bl_choice)-1) )
		{
			if (bl_choice[0] =='1')
			{
				/* Transfert fields from horig_note to hMes */
				if ( CopyFieldListFromOneNoteToAnother(hBL_Note, "Notif_MessageAppend_Field_Names", horig_note, hMes) == NOERROR )				
					ProcBl_AppendFieldDetailsToEmailBody(hBL_Note, hMes, "Notif_MessageAppend_Field_Titles", "Notif_MessageAppend_Field_Names");
			}
		}
...
void NonDeliveryBlackList_DocLink(DBHANDLE hDB, NOTEHANDLE horig_note, char * from, char *sendto, char* sendtobis,
								  ...
								  char* Append_Orig_Body, DBHANDLE* hMb, char* Orig_from, char* Orig_To,
								  char* Original_Body_Attach, char* Original_MessageID)
{
	STATUS			erreur;
	NOTEHANDLE		hMes;
	HANDLE			phCompound;
	...
	if (strcmp(sendtobis, "NULL"))
	{
		/* 4.0.1.125 : Get the Correct 'SendTo', 'CopyTo', 'BlindCopyTo' field */
		GetAddressFieldFor(horig_note, sendtobis, AddressField);
		NSFItemAppendTextList(hMes, AddressField,     sendtobis, (WORD) strlen(sendtobis), FALSE);
	...
	if ( nbrec > 0 )
	{
		...
		/* Integration of 3.95.11 : MWSARC to capture $MessageID instead of DOCID for attachment sharing */
...
/*			NSFItemAppendTextList(hNote, fieldname, safe_recipient, (WORD) strlen(safe_recipient), FALSE); */
	...
	/* Do this only one time per rules.. as the 'BlackLister' and 'FROM' are the same */
 ...
 *
 * Revision : 4.1.1.123 : Contact Extension Size
 ********************************************************************/
	...
	while(error==NOERROR)
	{
		...
		if ((wType & TYPE_OBJECT) == TYPE_OBJECT)
		{
			...
			if (pFileObject.Header.ObjectType == OBJECT_FILE)
			{
				...
				/* 4.1.1.124 : Paranoia : Check Memory */
				if ( filename != NULL )
				{					
 ...
 * ---------------------------------------------------------------
 * Release 3.92
 * char	MW_DocID[DOCID];	Original Notes Document unique ID seen by a MailWatcher server
 * char	subject[MAXRECLEN]; mail subject "" if not activated
 * Release 4.1.1.123
 * char ExtSize[ ]; Attachment size
 ******************************************************************/
void dumpstat(void)
{
	unsigned int	i;
					 ...
					 HANDLE* hCompound)
{
	...
	if (error = NOERROR)
	{
		...
		if ( error == NOERROR )
		{
	 		error = NSFNoteOpen(hBck, View_ID, 0, &hView);
...
UpperCase(ExtensionList);
 ...
 for(i=0;;prev_bid = item_bid,i++)
 {
     ...
     {
		/* Work out */
		NSFGetAttachmentFileName(value_bid, FileName, FileNameExtension, sizeof(FileName));
...
STATUS UpdateNewMailNote(NOTEHANDLE hNote)
{
	...
	NSFItemDelete(hNote, "MW_SKIPRULE",       11);
	NSFItemDelete(hNote, MW_MAILDUPLICATE, (WORD) strlen(MW_MAILDUPLICATE));
	/* 3.94.56 : NSFItemDelete(hNote, MW_MACRO_FIELD_NB_RECIPIENTS,(WORD) strlen(MW_MACRO_FIELD_NB_RECIPIENTS)); */ /* 3.94.46 */
	NSFItemDelete(hNote, MW_MACRO_FIELD_RECIPIENTS, (WORD) strlen(MW_MACRO_FIELD_RECIPIENTS)); /* 4.0.1.105 */
...
int UpdateRecipientsList(NOTEHANDLE hNote)
{
	...
	if ( MWBLACKLISTDEBUG == TRUE )
		AddInLogMessageText("--------[ END : final Recipients count: '%d']--------", NOERROR, nbrec);
...
/* #ifdef VERBOSE */
			if ( MWBLACKLISTDEBUG == TRUE )
				AddInLogMessageText("Recipients field - appending '%s'", NOERROR, recipient);
/* #endif */
		}
	}
...
* if returns (1) = skip the rule, time slot defined and no matching
***************************************************************************************/
int	CheckTimeSlotRules(NOTEHANDLE note_handle)
{
	int	skipThisRule = 0;
	char MW_RAPP_Flag[1024];

	/* if not timeslot defined, abort, return 0 */
	if ( NSFItemGetText(note_handle, BLACKLIST_TIME_SLOT_DAYHOURS, MW_RAPP_Flag, sizeof(MW_RAPP_Flag)-1) )
	{
		if ( MWBLACKLISTDEBUG == TRUE )
			AddInLogMessageText("Checking Timeslot '%s'", NOERROR, MW_RAPP_Flag);

		skipThisRule = !(nowDateMatchTimeIntervalString(MW_RAPP_Flag));
	}
...
STATUS GetUserAllAlias(char* username, GrpList* GrpHistory)
{
	...
	if (GrpHistory != NULL)
	{
		...
		AliasFound =              GetUserAlias(username, "shortname",       GrpHistory);
		AliasFound = AliasFound + GetUserAlias(username, "FullName",        GrpHistory);
		AliasFound = AliasFound + GetUserAlias(username, "InternetAddress", GrpHistory);
		/*3.93.27 : use Internet Forward address too */
		AliasFound = AliasFound + GetUserAlias(username, "MailAddress",     GrpHistory);
		AliasFound = AliasFound + GetUserAlias(username, "ccMailLocation",  GrpHistory);

		AliasFound = AliasFound + GetUserAlias(username, "AltFullName",     GrpHistory);

		/* 3.94.50 : ----------- Predicats  -------------------------------------------*/
		/* GetUserAlias(username, "CompanyName",     GrpHistory); */
		/* Loop over Predicat list defined in the rules */
		/* Save as [ ##_PREDICATS_<FIELDNAME> | value ( mono ) ] */
		/* Search for Predicats, if found search for FROM or TO if member of list */
		/* if found (FROM/TO); then return | value ( mono) - TEXT */
		/*-----------------------------------------------------------------------------*/
...
*			: temporay fields
*********************************************************************/
STATUS SplitMail(DBHANDLE* hMB, NOTEHANDLE hNote, NOTEHANDLE* hCopy, char* FieldRecipientName)
{
	...
	/* Purge MailWatcher temporary working field (RecOK, RecNotOK) fields */
	/* 4.0.1.115 : Recipient field can be configured */
	NSFItemDelete(*hCopy, FieldRecipientName /* MAILNOTE_RECIPIENTS_FIELD */, (WORD) strlen(FieldRecipientName));
...
* OUTPUT	: STATUS
************************************************************/
void NSFGetAttachmentFileName(BLOCKID bidv, char* FileName, char* FileNameExtension, WORD MaxSize)
{
	STATUS		error = NOERROR;
	void		*ptrItemValue;
	WORD		*ptrItemValueWORD;
...
STATUS	CheckRecipientField(DBHANDLE hmb, NOTEHANDLE hNote, char* FieldName, WORD* nbUserFound, char * FromValue)
{
	...
	if (Grp == NULL)
	{
		AddInLogMessageText("could not get enough memory for Group/recipients item %s ", NOERROR, "CheckRecipientField");
		return(error);
	}
	...
	*nbUserFound	= 0;
	...
	for (j=0; j<nbrec; j++)
	{
		...
		error =	GetAllAddressesFromField(hNote, username, FromValue, nbUserFound, L, P, 0, Grp, GrpP);
	}
	...
	if ( *nbUserFound > 0)
	{
		...
		/* 4.0.1.115 : the official Recipient field could be replaced by another field */
		NSFItemDelete(hNote, FieldName /* MAILNOTE_RECIPIENTS_FIELD */, (WORD) strlen(FieldName));
			...
			if ( P != NULL )	/* 3.93.8 */
			{
				...
				if ( username != NULL )	/* 3.93.8 */
				{
					...
					if (CurrentSize + UserSize > MAX_RECIPIENTS_FIELD_SIZE)
					{
						...
						error = SplitMail(&hmb, hNote, &hNewNote, FieldName);
						if (error == NOERROR)
						{
							hNoteSafe   = hNote;
							hNote       = hNewNote;
					...
					*/
					...
					/* 4.0.92 : TODO : stop calling parseSMTPaddress - it does not support "abc"<abc.test.com>@domainNotes - it removes the Notes Domain */
					/* parseSMTPaddress(username, username); */

					/* 4.0.1.115 : Recipient field can be configured */
					NSFItemAppendTextList(hNote, FieldName /* MAILNOTE_RECIPIENTS_FIELD */, username, (WORD) strlen(username), TRUE);
...
/********************************************************************************************************
* PROG		: JournalEmail_ExpandGroups
* PURPOSE	: for email going to journal DB. expands all Group from Recipients field
*			: From Saved version of 'Original Recipients fields'. ( $JournalRecipients  )
*			: Create and $JournalRecipientsExpanded_n ( Size > 15KB / field )
* -------------------------------------------------------------------------------------------------------
* TODO		: work out from a Safe copy of Recipient  "MW_Recipients" -> #define MW_MACRO_FIELD_RECIPIENTS
********************************************************************************************************/
STATUS	JournalEmail_ExpandGroups(DBHANDLE hmb, NOTEHANDLE hNote, char* FieldName, char* FromValue)
{
	STATUS		error = NOERROR;
	List		L;	/* list.h */
	Position	P;	/* list.h */
	List		Grp;
	Position	GrpP;
	WORD		nbrec, j, CurrentSize, UserSize, nbUserFound ;
	char		username[MAXUSERNAME];
	char		MailDuplicate[4];
	int			NbField = 1;
	char		JournalRecipient_FieldName[MAXUSERNAME];
	int			ElementCount = 0;
	...
	if ( L == NULL )
	{
		AddInLogMessageText("MWADVT: could not get enough memory for recipients item %s ", NOERROR, "JournalEmail_ExpandGroups");
		return(error);
	}
	...
	/* 4.0.1.103 : FromValue will be compared - UPPERCASE .. */
	UpperCase(FromValue);

	/* Read :  Loop over field values */
	nbrec = NSFItemGetTextListEntries(hNote, FieldName);
	...
	for ( j=0; j<nbrec; j++ )
	{
		...
		/* Write one by one into '$JournalRecipients' */
		if ( j == 0 )
			NSFItemSetTextSummary(hNote, MAILNOTE_JOURNAL_REC, username, (WORD) strlen(username), FALSE);	/* 4.0.1.103 : FALSE : NON SUMMARY */
		else
			NSFItemAppendTextList(hNote, MAILNOTE_JOURNAL_REC, username, (WORD) strlen(username), TRUE);

		/* Expand - get all user addresses */
		error =	GetAllAddressesFromField(hNote, username, FromValue, &nbUserFound, L, P, 0, Grp, GrpP);
	}

	/* Loop over List write back to "$JournalRecipientsExpanded_n", free memory */
	/* 4.0.1.103 : create only if a group has been expanded.. more user found compared to original Recipients field */

	if ( nbUserFound > nbrec )
	{
		/* Loop over List write back to "recipients", free memory - deduplication made with 'NSFItemAppendTextList' */
		...
		do
		{
			...
			if ( P != NULL )
			{
				...
				if ( username != NULL )
				{
					...
					if ( CurrentSize + UserSize > MAX_RECIPIENTS_FIELD_SIZE )
					{
						/* increment field count :  $JournalRecipientsExpanded_n */
						NbField++;						
						CurrentSize = 2; /* see declaration */	
						ElementCount = 0;
					}
					...
					/* build field name */
					sprintf(JournalRecipient_FieldName, "%s%d", MAILNOTE_JOURNAL_EXPAND_REC, NbField);
					if ( ElementCount == 0 )
					{
						NSFItemSetTextSummary(hNote, JournalRecipient_FieldName, username, (WORD) strlen(username), FALSE);	/* 4.0.1.103 : last FALSE, to force NON SUMMARY */
						ElementCount++;
					}
					else
						NSFItemAppendTextList(hNote, JournalRecipient_FieldName, username, (WORD) strlen(username), FALSE); /* 4.0.1.103 : do not allow duplicate ( last argt ) */
...
/* end if JournalEmail_ExpandGroups */
...
/*************************
* 
* 4.1.102 : Deja Vu : Group !!
*
*************************/
STATUS GetAllAddressesFromField(NOTEHANDLE hNote, char* temp, char* FromValue, WORD* nbUserFound, List UserAddresses, Position P, 
								int recurse, List Grp, Position GrpP)
{
	STATUS		error = NOERROR;
	HANDLE		hLookup = NULLHANDLE;
	char		*pLookup, *pName, *pMatch;
   ...
   */
	...
	do
	{
		...
		{
			/* 4.0.1.102 : DejaVu ?? */
			if ( Find( temp2, GrpP ) == NULL ) 
			{
				/* 4.1.0.102 : Insert recent expanded Group in List */
				Insert(temp2, Grp, GrpP);

				GetAllAddressesFromField(hNote, temp2, FromValue, nbUserFound, UserAddresses, P, recurse, Grp, GrpP);
			}
		}
		else
		{
...
STATUS GetDominoHostName(DBHANDLE module_dbnab, char * capath, char* ServerName, char* Serverhostname)
{
	...
	/* Open view  */
	/* 4.0.89 : do not need to recreate the index : OPEN_NOUPDATE */
	if (error = NIFOpenCollection(dbnab, dbnab, nab_server_view_id, OPEN_NOUPDATE /* OPEN_REBUILD_INDEX */,
								  NULLHANDLE,
								  &server_view_coll,
								  NULL, NULL, NULL, NULL))
	{
...
* OUPUT		: STATUS
**********************************************************************************/
STATUS	NSFItemDuplicateTextListNonSummary(NOTEHANDLE hNote, char* Field_in, char* Field_out, BOOL AllowDuplicateItem)
{
	STATUS	error = NOERROR;
	int		k = 0;
	...
	if ( NSFItemIsPresent(hNote, Field_in, (WORD) strlen(Field_in) ))
	{
		...
		if ( nbval > 0 )
		{
			...
			for ( i = 0; i < nbval; i++)
			{
				if ( NSFItemGetTextListEntry(hNote, Field_in, i, LocalRec, MAXUSERNAME-1))
				{
					/* First time : set NON SUMMARY flag */
					k++;
					if ( k == 1 )
					{
						if ( error = NSFItemSetTextSummary(hNote, Field_out,  LocalRec, (WORD) strlen(LocalRec) ,FALSE))
							i = nbval;
					}
					else
					{
...
***********************************************************************************/
int	CheckBlackListRulesMemoryValidation(NOTEHANDLE hNote, RulesList* L_rules, char* RulesRef, int MWBLACKLISTDEBUG )
{
	STATUS		error = NOERROR;
	double		result;
	char		RulesCmd[BLACKLIST_RULES_MEMORY_MAX_CMD];
	char		RulesCmd_display[BLACKLIST_RULES_MEMORY_MAX_CMD];
	...
	if ( NSFItemGetText(hNote, BLACKLIST_RULES_MEMORY_REF_CMD, RulesCmd, BLACKLIST_RULES_MEMORY_MAX_CMD -1 ) )
	{
		/* 4.0.1.109 */
		if ( MWBLACKLISTDEBUG )
			strcpy(RulesCmd_display, RulesCmd);

		if ( evalrules(L_rules, RulesCmd, &result, MWBLACKLISTDEBUG) )
			return( 0 );

		if ( result == 1)
		{
			if ( MWBLACKLISTDEBUG )
				AddInLogMessageText("Actions for '%s', count +1 for '%s'", NOERROR, RulesCmd_display, RulesRef);
										...
										int iUnderline, char sep)
{
	...
	switch ( nbitem )
	{
		...
		case 1:
			/* 4.1.1.127 : Extend to 90 - for a single */
			sprintf(sHeaderTitle,"! %90s !", sTitle1);
		...
		case 2:
			/* 4.1.1.127 : Extend to 50*2 - for two  : %-50s : left alignment */
			sprintf(sHeaderTitle,"! %50s ! %-50s !", sTitle1, sTitle2);
		...
		for (i = 0; i < nbitem; i++)
		{
			if ( sFieldValue1[i] != NULL )
			{
				...
				if ( error == NOERROR )
				{
					...
					if ( LongLine != NULL )
					{
						sprintf(LongLine,"! %50s ! %-50s !", sFieldValue1[i], sFieldValue2[i]);
						error = AppendLineToDefinedBodyField(hCompound, dwNormalStyle, LongLine);
						free(LongLine);
					}
				}
 ...
 *	LogErr() :
 *
 *	INPUT    : char * = message : internal message to be printed with the error
 *			 : STATUS = code erreur interne Notes
 * INPUT/OUT : char*  = error message
 *****************************************************************************************/
void LogErr(char *message, STATUS err)
{

	char meserr[MAX_DOC_FIELD_TEXT_SIZE];

	if (err)
	{
		OSLoadString(NULLHANDLE, ERR(err), meserr, sizeof(meserr)-1);
		LogEventText("error in %s : '%s'- '%s'", NULLHANDLE, NOERROR, NOMAPI, message, meserr);
	}
	else
...
STATUS	MailInForJournaling(DBHANDLE hMB, NOTEHANDLE hNote, char * MailInAddress)
{
	...
	char			NewFromAddress[MAXUSERNAME];
	int				KeepFromAddress = 1;	/* 4.0.87 : test */
	char			FromValue[MAXUSERNAME];	/* 4.0.1.102 */
	...
	NSFItemDuplicateTextListNonSummary(hNewNote, MAILNOTE_BLINDCOPY_FIELD, "JRN_BlindCopyTo", FALSE /* BOOL AllowDuplicateItem */);

	/* 4.0.91 : STOP DOING : 	NSFItemDuplicateTextList(hNewNote, MAILNOTE_FROM_FIELD, "JRN_From", FALSE);  */

	/* (3) : 4.1.0.102 : Save Current Original Recipients field + Expand if necessary */
	NSFItemGetText(hNewNote, MAILNOTE_FROM_FIELD, FromValue, MAXUSERNAME -1);
	JournalEmail_ExpandGroups(hMB, hNewNote, MW_MACRO_FIELD_RECIPIENTS, FromValue);
	...
	/* 4.0.81 : FROM != Recipient - when FROM == Recipients and FROM is Group then router does not deliver the mail */
	if ( (strlen(MailInAddress) + 4) < sizeof(NewFromAddress) )
		sprintf(NewFromAddress,"JRN_%s", MailInAddress);
	else
		strcpy(NewFromAddress, alertfrom /* Global variable */);
	
	/* 4.0.87 */		 
	if ( KeepFromAddress != 1 )
		NSFItemSetTextSummary(hNewNote, MAILNOTE_FROM_FIELD, NewFromAddress, (WORD) strlen(NewFromAddress), FALSE );

	/* 4.0.88 : prevent from boucing using native journaling trick : $JournalResponsibility="2" */
	NSFItemSetTextSummary(hNewNote, "$JournalResponsibility", "2", 1, FALSE );			/* 4.0.1.107 : NON SUMMARY */
	...
	NSFItemDelete(hNewNote, MW_MACRO_FIELD_NB_RECIPIENTS,(WORD) strlen(MW_MACRO_FIELD_NB_RECIPIENTS));
	NSFItemDelete(hNewNote, MW_MACRO_FIELD_RECIPIENTS,   (WORD) strlen(MW_MACRO_FIELD_RECIPIENTS));
	...
	if ( error )
	{
		...
		AddInLogMessageText(ERR_SAVE_JOURNAL_DOC, error, NOMAPI, "mail(x).box");			/* 4.0.1.107 : error for SAVE */
		NSFItemSetText(hNote, "FAILUREREASON", ERR_JOURNAL_FAILURE_REASON, (WORD) strlen(ERR_JOURNAL_FAILURE_REASON));
...
/*************************************************************************************************************************
* PROG : CopyFieldListFromOneNoteToAnother
* PURPOSE	: Based on a field value ( TEXT LIST ) form the hNoteFiltre, 
*			: Copy one by one all the FIELD from Orig to hMes
*************************************************************************************************************************/
STATUS CopyFieldListFromOneNoteToAnother(NOTEHANDLE hNoteFiltre, char* sFieldRef, NOTEHANDLE horig_note, NOTEHANDLE hMes)
{
	STATUS	error = NOERROR;
	char*	sFieldValue[MAXDELETEATTACH];
	int		nbitem = 0;
	...
	for ( j = 0; j < nbitem; j++ )
	{
		if ( sFieldValue[j] != NULL )
			error = MoveFieldFromNote2Note(horig_note, hMes, sFieldValue[j]);
...
/*******************************************************************************************************************
* PROG		: GetMacroAndReplace()
* PURPOSE	: Check in string the pattern : {FIELDNAME} or {TO}
*
********************************************************************************************************************/
STATUS GetMacroAndReplace(NOTEHANDLE hNote, char* in_string, char* out_string, int max_len)
{
	STATUS	error = NOERROR;
	char	fieldname[MAXUSERNAME];
	char*	pos1 = NULL;
	char*	pos2 = NULL;

	/* check for {*} pattern with a MAXUSERNAME string max in between */
	pos1 = strstr(in_string, "{");
	...
	if ( pos1 != NULL )
	{
		...
		if ( pos2 != NULL )
		{
			...
			if ( ( pos2 - pos1 ) < MAXUSERNAME )
			{	
				strncpy(fieldname, pos1+1, pos2 - pos1);
				fieldname[pos2-pos1-1] = '\0';  /* Close fieldname string */
				ReplaceMacroFieldinText(hNote, fieldname, in_string, out_string, max_len);
				GetMacroAndReplace(hNote, pos2+1 /* skip the '}' */, out_string, max_len);
			}
		}
	}
	else
		strcat(out_string, in_string);
...
/*********************************************************************************************************************
* PROG		: ReplaceMacroFieldinText
* PURPOSE	: use the HNOTE and Field name detected in the text string
* EXAMPLE	: Hello, the recipient {TO} of this email, {SUBJECT} has left the company...
*			: {TO} : replaced by the current Recipients(i);
*			: {FIELDNAME} 
* Depend on : STATUS MakeStringFromNotesAddressList(NOTEHANDLE hNote, char* fieldname, char* out_string, int max_len)
***********************************************************************************************************************/
STATUS	ReplaceMacroFieldinText(NOTEHANDLE hNote, char* fieldname, char* in_string, char* out_string, int max_len)
{
	STATUS	error = NOERROR;
	char	FieldMarker[MAXUSERNAME];
	char	tmp_out_string[1024];			/* 4.0.1.120 : Static for now... */
	char*	pos;
	char*	orig;
	...
	/* Find the place of the field macro into the string */
	sprintf(FieldMarker, "{%s}", fieldname);
	MarkerLength = strlen(FieldMarker);
	pos = strstr(in_string, FieldMarker );
	
	if ( pos != NULL )
	{
		/* strcat : Copy original string up to pos */
		strncpy(tmp_out_string, in_string, pos - orig);
		tmp_out_string[pos-orig]='\0';
		strcat(out_string, tmp_out_string);
		
		/* Append field value */
		error = MakeStringFromField(hNote, fieldname, out_string, max_len);	
	}
	else
...
/*******************************************************************************************
PROG	: MakeStringFromField()
PURPOSE	: Convert a field to a string
		: Add the value directly at the end of out_string.
		:  - NOT SUPPORTED -
		: TYPE_NUMBER_RANGE
		: TYPE_TIME_RANGE
*********************************************************************************************/
STATUS	MakeStringFromField(NOTEHANDLE hNote, char* fieldname, char* out_string, int max_len)
{
	...
	size_t		max_len_out	= 0;

	/* Type ? */
	NSFItemInfo(hNote, fieldname, (WORD) strlen(fieldname), NULL, &typechamp, NULL, NULL);

	/* Get current Size : Adjust max_len_value */
	current_out_size = strlen(out_string);
	max_len_out = (size_t) max_len - current_out_size;

	/* Alloc anyway  - even if the type is not supported */
	FieldValue = (char*) malloc(max_len_out * sizeof(char) );
	if (FieldValue == NULL )
		return(ERR_ITEM_NOT_FOUND);


	/* TEXT - Missing SMTP header : 4.1.1.128 */
	if ( ( typechamp == TYPE_TEXT ) || ( typechamp == TYPE_RFC822_TEXT ) )
	{
		if ( NSFItemGetText(hNote, fieldname, FieldValue, max_len_out-1) )
			strcat(out_string, FieldValue);
		else
			error = ERR_ITEM_NOT_FOUND;
	}
	...
	if ( typechamp == TYPE_TEXT_LIST )
	{
		error = MakeStringFromNotesAddressList(hNote, fieldname, FieldValue, max_len_out);
		if ( error == NOERROR )
			strcat(out_string, FieldValue);		
	}
	...
	/* DATE */
	else if ( typechamp == TYPE_TIME )
	{
		if ( NSFItemGetTime(hNote, fieldname, &tmpTimeDate.GM) )
		{
			TimeGMToLocal(&tmpTimeDate);
			sprintf(FieldValue, "(YYYY/MM/DD) : %04d/%02d/%02d", tmpTimeDate.year, tmpTimeDate.month, tmpTimeDate.day);
			strcat(out_string, FieldValue);
		}
		else
			error = ERR_ITEM_NOT_FOUND;
	...
	/* NUMBER */
	else if ( typechamp == TYPE_NUMBER )
	{
		if ( NSFItemGetNumber(hNote, fieldname, &tmpNumber) )
		{
			sprintf(FieldValue ,"%d", tmpNumber);
			strcat(out_string, FieldValue);
		}
		else
			error = ERR_ITEM_NOT_FOUND;
...
/***********************************************************************************************
* Prog	: GetAddressFieldFor()
* Purpose	: Get the field name where 'SendToBis' has been defined
*
* using global var for optimization these vars are init once per message during ProcBlackList init
* TempAddressConcatSendTo[0] = '\0';
* TempAddressConcatCopyTo[0] = '\0';
* TempAddressConcatBlindCopyTo[0] = '\0'; 
************************************************************************************************/
STATUS	GetAddressFieldFor(NOTEHANDLE horig_note, char* sendTobis, char* AddressField)
{
	...
	/* default */
	strcpy(AddressField, "SendTo");


	/* Check TO */
	if ( strlen(TempAddressConcatSendTo) == 0 )
		error = MakeStringFromNotesAddressList(horig_note, "SendTo", TempAddressConcatSendTo, MAX_DOC_FIELD_TEXT_SIZE -1);

	if ( error == NOERROR )
	{
		if ( strstr(TempAddressConcatSendTo, sendTobis) )
			return(NOERROR);
	}
	else
		return(error);
	...
	/* Check CC */
	if ( strlen(TempAddressConcatCopyTo) == 0 )
		error = MakeStringFromNotesAddressList(horig_note, "CopyTo", TempAddressConcatCopyTo, MAX_DOC_FIELD_TEXT_SIZE -1);
	
	if ( error == NOERROR )
	{
		if ( strstr(TempAddressConcatCopyTo, sendTobis) )
		{
			strcpy(AddressField, "CopyTo");
			return(NOERROR);
		}
	}
	else
	...
	/* Check BCC */
	if ( strlen(TempAddressConcatBlindCopyTo) == 0 )
		error = MakeStringFromNotesAddressList(horig_note, "BlindCopyTo", TempAddressConcatBlindCopyTo, MAX_DOC_FIELD_TEXT_SIZE -1);

	if ( error == NOERROR )
	{
		if ( strstr(TempAddressConcatBlindCopyTo, sendTobis) )
		{
			strcpy(AddressField, "BlindCopyTo");
			return(NOERROR);
		}
	}
...
/*************************************************************************************************
PROG		: AppendToTextListWithStringAndSeparator
PURPOSE		: Get a string "a,b,c,d" - separator hard coded
			: AppendToTextList - NOTEHANDLE and FIELD name in parameters
			:
			: Return STATUS
*************************************************************************************************/
STATUS	AppendToTextListWithStringAndSeparator(NOTEHANDLE hNote, char * fieldName, char* fieldValues)
