 ...
 * 31/05/06				: - 
 *						: revision 3.93.44.
 *						: fix in ProcBlackList : DeliveryPriority instead of Importance.
 *						: DeliveryPriority = 'l' : low priority.
 *						:
 * 16/11/06				: revision 3.93.45-46
 *						: NIFOpenCollection( 4th argt reset to ZERO -all the time ).
 *						:
 * 25/05/07				: revision 3.93.47
 *						: SMLN 3.X support/compatibility issues.
 * --[ TO DO ]-------------------------------------------------------------------------------------------------------
 **************************************************************************************************/
...
STATUS GetDominoHostName(DBHANDLE dbnab, char * capath, char* ServerName, char* Serverhostname)
{
	...
	/* Open view  */
	if (error = NIFOpenCollection(dbnab, dbnab, nab_server_view_id, 0 /* 3.93.45 OPEN_REBUILD_INDEX */,
								  NULLHANDLE,
								  &server_view_coll,
								  NULL, NULL, NULL, NULL))
	{
