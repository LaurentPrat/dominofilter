...
double GetRulesFrequency(RulesList* L, char* sRules, double Interval, int MWBLACKLISTDEBUG)
{
	...
	/* Get the rules */
	if ( rItem != NULL )
		return(RulesFrequency(rItem->r, Interval, MWBLACKLISTDEBUG ));
...
double	RulesFrequency(RulesMemory* r, double Interval, int MWBLACKLISTDEBUG  )
{
	...
	/* 4.0.1.109 - 
	if Last time is really OLD then the action will not be executed
	because the frequency will be too SMALL, we should limit and reset Has_run and last time to NOW... */
	nb_second_last_run = ( long) ( time(NULL) - r->last_time );

	if ( nb_second_last_run > Interval )
	{
		r->last_time = r->start_time = time(NULL);
		r->has_run = 1;
		nb_second_last_run = 0;
	}

	/* elaspe in interval */
	nb_interval = (long) (nb_second_last_run / Interval);
	r->last_time = time(NULL);				/* 4.0.1.110 */
	...
	Nb_time = r->has_run;
	returnVal = (long) floor (Nb_time / nb_interval);

	if ( MWBLACKLISTDEBUG )
		AddInLogMessageText("'%s' [Elapse:%lu] [count:%lu] [return:%lu]",NOERROR, 
					r->RulesRef,
					nb_second_last_run, 
					Nb_time,
					returnVal);
...
* File format	: Dump format : "%s / %lu / %ld / %ld\n"
*				: (%s) Rules Ref
*				: (%d) Number of secondes since C ANSI T0 (1970)
*				: (%d) Last run time ( nb of secondes )
...
int RulesMemoryDumpToFile(RulesList* L, char* filename)
{
	...
	while ( L != NULL)
	{
		if ( L->r != NULL )
		{
			tmpHasRun    = L->r->has_run;
			tmpStartTime = ( long ) L->r->start_time;
			tmpLastTime  = ( long ) L->r->last_time;

			sprintf(buf,"%s / %ld / %ld / %ld \n", L->r->RulesRef, tmpStartTime, tmpLastTime, tmpHasRun );				/* 4.0.1.109 */
			fprintf(f, buf);
		}
	...
	while ( Header != NULL)
	{
		if ( Header->r != NULL )
			printf("Ref: %s ( %i ) [ %i ] -> %i \n", Header->r->RulesRef,
			                                  Header->r->start_time,
											  Header->r->has_run,
											  RulesFrequency(Header->r, Interval, FALSE /* disable debug switch */) );
		Header = Header->next;
	}
...
* File format	: Dump format : "%s / %lu / %lu \n"
*
*				: (%s) Rules Ref
*				: (%lu) Number of second since C ANSI T0 (1970)
*				: (%i) Number of time (int)
*
...
int RulesMemoryRestoreFromFile(RulesList* L, char* filename, RulesList** header )
{
	...
	time_t			file_last_time = 0;	/* 4.0.1.110 */
	int				file_has_run = 0;
	...
	while ( fscanf(f,"%s / %lu / %lu / %li ", file_ref, &file_start_time, &file_last_time, &file_has_run) != EOF )
	{
		/* insert */
		...
		if ( r1 != NULL )
		{
			...
			if ( file_start_time > 0 )
				r1->r->start_time	= file_start_time ;		/* 4.0.1.109 : protect against stupidity */
			else
				r1->r->start_time = time(NULL);				/* 4.0.1.109 : 01/01/1970 */

			/* Start time for Frequency readjustment */
			if ( file_last_time > 0 )
				r1->r->last_time	= file_last_time ;		/* 4.0.1.110 */
			else
				r1->r->last_time = 0;
...
int evalrules(RulesList* L, char* cmd, double* result, int MWBLACKLISTDEBUG)
{
	...
	switch ( f_cmd[0] )
	{
		...
		case 'F':
			result1 =  GetRulesFrequency(L, f_rulesref, f_val, MWBLACKLISTDEBUG);
		break;
